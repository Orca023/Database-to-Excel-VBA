...
// https://mariadb.com/ 數據庫操作第三方包「mariadb」官網;
// https://mariadb.com/docs 數據庫操作第三方包「mariadb」官方手冊;
...
// https://mariadb.com/docs/release-notes/connectors/node.js 數據庫操作第三方包「mariadb」官網;
// https://mariadb.com/docs/connectors/mariadb-connector-nodejs/mariadb-connector-node-js-guide 官網下載地址 mariadb 數據庫服務器軟件;
// https://www.npmjs.com/package/mariadb 數據庫擴展包 mariadb 説明;
// https://github.com/MariaDB/
// https://github.com/MariaDB/server
// https://www.w3ccoo.com/mariadb/index.html 數據庫 mariadb 使用 W3C 網教程;

// 需要事先在控制臺命令行安裝第三方擴展包;
// root@localhost:~# npm install mariadb --registry=https://registry.npm.taobao.org
// root@localhost:~# npm install mysql --registry=https://registry.npm.taobao.org
...
// const redis = require('redis'); //引用第三方擴展包「redis」用於與操作内存數據庫服務器 redis，需要先「npm install redis@version --prefix=./node_modules --registry=https://registry.npm.taobao.org/」安裝成功才能使用;
// const mysql = require('mysql'); //引用第三方擴展包「mysql」用於與 mysql 數據庫服務器通信進行數據操作，需要先「npm install mysql@version --prefix=./node_modules --registry=https://registry.npm.taobao.org/」安裝成功才能使用;
const mariadb = require('mariadb/callback'); //引用第三方擴展包「mariadb」用於與 MariaDB 數據庫服務器通信進行數據操作，需要先「npm install mariadb@version --prefix=./node_modules --registry=https://registry.npm.taobao.org/」安裝成功才能使用;
...
function do_Request_Router(
    ...
    // UserPass = dbUser.concat(":", dbPass);  // 'admin_Collection1:admin';  // ['root:root', 'administrator:administrator', 'admin_Collection1:admin', 'user_Collection1:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
    if (url_search_JSON.has("dbName")) {
        dbName = String(url_search_JSON.get("dbName"));  // 'Database1'; // ['admin', 'Database1'];  // 定義數據庫名字變量用於儲存數據庫名，將數據庫名設為形參，這樣便於日後修改數據庫名，MariaDB 要求數據庫名稱首字母必須為大寫單數;
    };
    if (url_search_JSON.has("dbTableName")) {
        dbTableName = String(url_search_JSON.get("dbTableName"));  // 'Collection1'; // ['Collection1'];  // MariaDB 數據庫包含的數據集合（表格）;
    };
    // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
    // [*, Field-name-1, Field-name-2, Field-name-3, ... ];
    let selectFieldName = [];
    if (url_search_JSON.has("selectFieldName")) {
        // console.log(url_search_JSON.get("selectFieldName"));
        if (Object.prototype.toString.call(url_search_JSON.get("selectFieldName")).toLowerCase() === '[object string]') {
            if ((url_search_JSON.get("selectFieldName") === "") || (url_search_JSON.get("selectFieldName") === "\[\]") || (url_search_JSON.get("selectFieldName") === "\"\"") || (url_search_JSON.get("selectFieldName") === "\'\'") || (url_search_JSON.get("selectFieldName") === "\`\`")) {
                selectFieldName = [];
            } else if ((url_search_JSON.get("selectFieldName") === "\*") || (url_search_JSON.get("selectFieldName") === "\[\*\]") || (url_search_JSON.get("selectFieldName") === "\[\"\*\"\]") || (url_search_JSON.get("selectFieldName") === "\[\'\*\'\]") || (url_search_JSON.get("selectFieldName") === "\[\`\*\`\]")) {
                selectFieldName = ["*"];
            } else {
                selectFieldName = JSON.parse(url_search_JSON.get("selectFieldName"));  // ["*", "Field-name-1", "Field-name-2", "Field-name-3", ... ];
            };
            if ((Object.prototype.toString.call(selectFieldName).toLowerCase() === '[object array]') && (selectFieldName.length > 0)) {
                for (let i = 0; i < selectFieldName.length; i++) {
                    // console.log(selectFieldName[i]);
                    if (!((typeof (selectFieldName[i]) === 'undefined') || ((typeof (selectFieldName[i]) === 'object') && (selectFieldName[i] === null)))) {
                        selectFieldName[i] = String(selectFieldName[i]);
                    };
                };
            };
        };
    ...
    // // 判斷目標資料庫（Database）名稱變量：dbName 是否爲空，如果爲空，則中止退出;
    // if ((typeof (dbName) === 'undefined') || ((typeof (dbName) === 'object') && (dbName === null)) || ((Object.prototype.toString.call(dbName).toLowerCase() === '[object string]') && (dbName === ""))) {
    //     // 判斷目標資料表（Table/Collection）名稱變量：dbTableName 是否爲空，如果爲空，則中止退出;
    //     if ((typeof (dbTableName) === 'undefined') || ((typeof (dbTableName) === 'object') && (dbTableName === null)) || ((Object.prototype.toString.call(dbTableName).toLowerCase() === '[object string]') && (dbTableName === ""))) {
    //         console.log("Database and Table/Collection name requested empty.");
    //         response_data_JSON["Database_say"] = String("Database and Table/Collection name requested empty.");
    //         response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
    //         // String = JSON.stringify(JSON); JSON = JSON.parse(String);
    //         if (callback) { callback(null, response_body_String); };
    //         return response_body_String;
    //     } else {
    //         console.log("Database name requested empty.");
    //         response_data_JSON["Database_say"] = String("Database name requested empty.");
    //         response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
    //         // String = JSON.stringify(JSON); JSON = JSON.parse(String);
    //         if (callback) { callback(null, response_body_String); };
    //         return response_body_String;
    ...
    //     // 判斷目標資料表（Table/Collection）名稱變量：dbTableName 是否爲空，如果爲空，則中止退出;
    //     if ((typeof (dbTableName) === 'undefined') || ((typeof (dbTableName) === 'object') && (dbTableName === null)) || ((Object.prototype.toString.call(dbTableName).toLowerCase() === '[object string]') && (dbTableName === ""))) {
    //         console.log("Table/Collection name requested empty.");
    //         response_data_JSON["Database_say"] = String("Table/Collection name requested empty.");
    //         response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
    //         // String = JSON.stringify(JSON); JSON = JSON.parse(String);
    //         if (callback) { callback(null, response_body_String); };
    //         return response_body_String;
    ...
    // 數據庫程式 MariaDB 待請求執行的 SQL 語句字符串;
    let SQL_String = "";
    switch (request_url_path) {
        ...
        // 向 MariaDB 數據庫伺服器裏指定數據庫裏新創建指定集合（表格）;
        // URL = "/createCollection?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { "table_name" : "\`targetTableNameString\`" }
        // request_POST_JSON = [{ "table_name" : "\`targetTableNameString-1\`" }, { "table_name" : "\`targetTableNameString-2\`" }, ... ]
        // CREATE TABLE `Collection1` (`AutoID` INT AUTO_INCREMENT PRIMARY KEY,`ReplicationID` TINYINT NOT NULL,`IntegerShort` SMALLINT,`IntegerLong` INT,`FloatSingle` FLOAT,`FloatDouble` DOUBLE,`TextShort` TINYTEXT NOT NULL,`TextLong` MEDIUMTEXT,`DateTime` DATETIME,`Date` DATE,`Time` TIME,`Currency` DECIMAL(10, 2) DEFAULT 0,`Image` LONGBLOB DEFAULT NULL,`VariableBinary` VARBINARY(8) DEFAULT NULL,`Array8Byte` BINARY(8) DEFAULT NULL,`Time_Stamp` TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide

            // 讀取用戶端（Client）發送請求（Post）的請求體（Body）資料;
            request_POST_JSON = {};
            // 自定義函數判斷客戶端 POST 請求傳送的 request_POST_String 是否為一個 JSON 格式的字符串;
            // if (request_POST_String !== "" && isStringJSON(request_POST_String)) {
                try {
            ...
            // 判斷 MariaDB 資料庫伺服器狀態;
            if (MariaDB_Client !== null) {

                // 判斷傳入變量：request_POST_JSON(Schema Field name or type) 數據，是否爲 JSON 類型;
                // SQL_String = "CREATE TABLE \`Collection1\` (\`AutoID\` INT AUTO_INCREMENT PRIMARY KEY,\`ReplicationID\` TINYINT NOT NULL,\`IntegerShort\` SMALLINT,\`IntegerLong\` INT,\`FloatSingle\` FLOAT,\`FloatDouble\` DOUBLE,\`TextShort\` TINYTEXT NOT NULL,\`TextLong\` MEDIUMTEXT,\`DateTime\` DATETIME,\`Date\` DATE,\`Time\` TIME,\`Currency\` DECIMAL(10, 2) DEFAULT 0,\`Image\` LONGBLOB DEFAULT NULL,\`VariableBinary\` VARBINARY(8) DEFAULT NULL,\`Array8Byte\` BINARY(8) DEFAULT NULL,\`Time_Stamp\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP);";
                if (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length)) {

                    // 讀取 request_POST_JSON 裏關鍵字（Key）爲「"collection_name"」的鍵值對的值（Value）;
                    if (request_POST_JSON.hasOwnProperty("collection_name")) {
                        // console.log(request_POST_JSON["collection_name"]);
                        if (!((typeof (request_POST_JSON["collection_name"]) === 'undefined') || ((typeof (request_POST_JSON["collection_name"]) === 'object') && (request_POST_JSON["collection_name"] === null)))) {
                            if (Object.prototype.toString.call(request_POST_JSON["collection_name"]).toLowerCase() === '[object string]') {
                                if ((String(request_POST_JSON["collection_name"]).length > 0) && (request_POST_JSON["collection_name"] !== "null") && (request_POST_JSON["collection_name"] !== "NULL") && (request_POST_JSON["collection_name"] !== "Null") && (request_POST_JSON["collection_name"] !== "none") && (request_POST_JSON["collection_name"] !== "NONE") && (request_POST_JSON["collection_name"] !== "None")) {
                                    dbTableName = String(request_POST_JSON["collection_name"]);
                                };
                            };
                        };
                    };
                    // 讀取 request_POST_JSON 裏關鍵字（Key）爲「"table_name"」的鍵值對的值（Value）;
                    if (request_POST_JSON.hasOwnProperty("table_name")) {
                        // console.log(request_POST_JSON["table_name"]);
                        if (!((typeof (request_POST_JSON["table_name"]) === 'undefined') || ((typeof (request_POST_JSON["table_name"]) === 'object') && (request_POST_JSON["table_name"] === null)))) {
                            if (Object.prototype.toString.call(request_POST_JSON["table_name"]).toLowerCase() === '[object string]') {
                                if ((String(request_POST_JSON["table_name"]).length > 0) && (request_POST_JSON["table_name"] !== "null") && (request_POST_JSON["table_name"] !== "NULL") && (request_POST_JSON["table_name"] !== "Null") && (request_POST_JSON["table_name"] !== "none") && (request_POST_JSON["table_name"] !== "NONE") && (request_POST_JSON["table_name"] !== "None")) {
                                    dbTableName = String(request_POST_JSON["table_name"]);
                                };
                            };
                        };
                    };
                    ...
                    if ((typeof (dbName) === 'undefined') || ((typeof (dbName) === 'object') && (dbName === null)) || ((Object.prototype.toString.call(dbName).toLowerCase() === '[object string]') && (dbName === ""))) {
                        ...
                        if ((typeof (dbTableName) === 'undefined') || ((typeof (dbTableName) === 'object') && (dbTableName === null)) || ((Object.prototype.toString.call(dbTableName).toLowerCase() === '[object string]') && (dbTableName === ""))) {
                            ...
                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                            if (callback) { callback(null, response_body_String); };
                            request_POST_JSON = {};  // = null;  // 歸零復位回收内存;
                            SQL_String = "";  // 初始化清零;
                            return response_body_String;
                        } else {
                            console.log("Database name requested empty.");
                    ...
                    // 拼接用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                    SQL_String = "";  // 初始化清零;
                    // SQL_String = "CREATE TABLE \`Collection1\` (\`AutoID\` INT AUTO_INCREMENT PRIMARY KEY,\`ReplicationID\` TINYINT NOT NULL,\`IntegerShort\` SMALLINT,\`IntegerLong\` INT,\`FloatSingle\` FLOAT,\`FloatDouble\` DOUBLE,\`TextShort\` TINYTEXT NOT NULL,\`TextLong\` MEDIUMTEXT,\`DateTime\` DATETIME,\`Date\` DATE,\`Time\` TIME,\`Currency\` DECIMAL(10, 2) DEFAULT 0,\`Image\` LONGBLOB DEFAULT NULL,\`VariableBinary\` VARBINARY(8) DEFAULT NULL,\`Array8Byte\` BINARY(8) DEFAULT NULL,\`Time_Stamp\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP);";
                    if (!((JSON.stringify(request_POST_JSON) === '{}') || ((Object.keys(request_POST_JSON).length === 1) && (request_POST_JSON.hasOwnProperty("selectFieldName") || request_POST_JSON.hasOwnProperty("table_name") || request_POST_JSON.hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON).length === 2) && ((request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name")) || (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("collection_name")) || (request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON).length === 3) && (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name"))))) {

                        // 拼接 SQL 語句字符串待新建的數據表（TABLE）名字符串;
                        SQL_String = "CREATE TABLE ";
                        SQL_String = SQL_String + "\`";
                        SQL_String = SQL_String + String(dbTableName);
                        SQL_String = SQL_String + "\`";
                        SQL_String = SQL_String + " (";

                        // 拼接 SQL 語句字符串待新建的數據表（TABLE）結構（Schema）字段（field）名和類型屬性字符串;
                        for (let key_POST in request_POST_JSON) {
                            // console.log(key_POST);
                            // console.log(request_POST_JSON[key_POST]);
                            if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0) && (String(request_POST_JSON[key_POST]).length > 0)) {
                                SQL_String = SQL_String + "\`";
                                SQL_String = SQL_String + String(key_POST);
                                SQL_String = SQL_String + "\`";
                                SQL_String = SQL_String + " ";
    
                                SQL_String = SQL_String + String(request_POST_JSON[key_POST]);
                                SQL_String = SQL_String + ",";
                            };
                        };

                        // 刪除 SQL 語句字符串末位多餘的逗號（,）字符;
                        if (String(SQL_String).length > 0) {
                            if (String(SQL_String.slice(-1)) === ",") {
                                SQL_String = String(SQL_String.slice(0, -1));
                            };
                        };

                        // 拼接 SQL 語句待新建的數據表（TABLE）結構（Schema）字段（field）名和類型屬性字符串末尾後半個括號（)）字符;
                        SQL_String = SQL_String + ")";

                        // 拼接 SQL 語句字符串末位結束符分號（;）字符;
                        SQL_String = SQL_String + ";";
                        console.log(SQL_String);

                        // 判斷 MariaDB 資料庫伺服器狀態，判斷拼接得 SQL 語句字符串合規性;
                        if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("CREATE TABLE ").length, 10))) {

                            // 向 MariaDB 資料庫伺服器發送 SQL 語句並執行操作請求（Request）然後接收響應（Response）值;
                            MariaDB_Client.query(
                                SQL_String,
                                function(
                                    error,  // 執行失敗時返回結果;
                                    results,  // 執行成功時返回結果;
                                    ...
                                    if (error) {
                                        ...
                                        response_data_JSON["Database_say"] = String(error.message);
                                        response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        if (callback) { callback(response_body_String, null); };
                                    ...
                                    if (results) {
                                        ...
                                        // response_body_String = JSON.stringify(results);
                                        response_data_JSON["Database_say"] = String("warningStatus: " + String(results.warningStatus));
                                        response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        if (callback) { callback(null, response_body_String); };
                            ...
                            console.log("Error, Input request_POST_JSON(Schema Field name or type) unrecognitized.");
                            response_data_JSON["Database_say"] = "Error, Input request_POST_JSON(Schema Field name or type) unrecognitized.";
                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                            if (callback) { callback(response_body_String, null); };
                        ...
                        console.log("Input request_POST_JSON(Schema Field name or type) empty.");
                        response_data_JSON["Database_say"] = "Input request_POST_JSON(Schema Field name or type) empty.";
                        response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                        // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                        if (callback) { callback(null, response_body_String); };
                    ...
                    console.log("Input request_POST_JSON(Schema Field name or type) unrecognitized, must be JSON format.");
                    response_data_JSON["Database_say"] = "Input request_POST_JSON(Schema Field name or type) unrecognitized, must be JSON format.";
                    response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                    // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                    if (callback) { callback(response_body_String, null); };
        ...
        case "/dropCollection": {
        // 從 MariaDB 數據庫伺服器裏指定數據庫裏刪除指定集合（表格）;
        // URL = "/dropCollection?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { "table_name" : "\`targetTableNameString\`" }
        // request_POST_JSON = [{ "table_name" : "\`targetTableNameString-1\`" }, { "table_name" : "\`targetTableNameString-2\`" }, ... ]
        ...
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            if (MariaDB_Client !== null) {
                ...
                if (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length)) {
                    ...
                    // 拼接用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                    SQL_String = "";  // 初始化清零;
                    // SQL_String = "DROP TABLE `Collection1`;";

                    // 拼接 SQL 語句字符串待刪除的數據表（TABLE）名字符串;
                    ...
                    // 判斷 MariaDB 資料庫伺服器狀態，判斷拼接得 SQL 語句字符串合規性;
                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("DROP TABLE ").length, 10))) {
        ...
        case "/insertOne": {
        // 向 MariaDB 數據庫伺服器裏指定數據庫裏的指定集合（表格）中插入數據;
        // URL = "/insertOne?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { "Key-field-1" : "Value-field-1", "Key-field-2" : "Value-field-2", ... }
        // request_POST_JSON = [{ "Key-field-1" : "Value-field-1", "Key-field-2" : "Value-field-2", ... }, { "Key-field-1" : "Value-field-1", "Key-field-2" : "Value-field-2", ... }, ... ]
        // request_POST_String = "{\"IntegerShort\":\"111\",\"IntegerLong\":\"111111\",\"FloatSingle\":\"111.111\",\"FloatDouble\":\"111111.111111\",\"TextShort\":\"aaa\",\"TextLong\":\"aaaaaa\",\"DateTime\":\"2021-1-1 11:01:01\",\"Date\":\"2021-1-1\",\"Time\":\"11:01:01\",\"Currency\":\"1.11\",\"Image\":\"\\\"\\\"\"}"
        // request_POST_String = "[{\"IntegerShort\":\"111\",\"IntegerLong\":\"111111\",\"FloatSingle\":\"111.111\",\"FloatDouble\":\"111111.111111\",\"TextShort\":\"aaa\",\"TextLong\":\"aaaaaa\",\"DateTime\":\"2021-1-1 11:01:01\",\"Date\":\"2021-1-1\",\"Time\":\"11:01:01\",\"Currency\":\"1.11\",\"Image\":\"\\\"\\\"\"},{\"IntegerShort\":\"222\",\"IntegerLong\":\"222222\",\"FloatSingle\":\"222.222\",\"FloatDouble\":\"222222.222222\",\"TextShort\":\"bbb\",\"TextLong\":\"bbbbbb\",\"DateTime\":\"2022-2-2 12:02:02\",\"Date\":\"2022-2-2\",\"Time\":\"12:02:02\",\"Currency\":\"2.22\",\"Image\":\"\\\"\\\"\"}]";
        // INSERT INTO `Collection1` (`ReplicationID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`) VALUES ("001","111","111111","111.111","111111.111111","aaa","aaaaaa","2021-1-1 11:01:01","2021-1-1","11:01:01","1.11",""),("002","222","222222","222.222","222222.222222","bbb","bbbbbb","2022-2-2 12:02:02","2022-2-2","12:02:02","2.22","");
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            if (MariaDB_Client !== null) {
                ...
                if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]' && request_POST_JSON.length > 0 && (typeof (request_POST_JSON[0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[0]).toLowerCase() === '[object object]' && !(request_POST_JSON[0].length) && JSON.stringify(request_POST_JSON[0]) !== '{}')) || (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length) && JSON.stringify(request_POST_JSON) !== '{}')) {
                    ...
                    // 判斷變量：request_POST_JSON 的數據類型，數組（Array）或 JSON 類型;
                    if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]') && (request_POST_JSON.length > 0)) {

                        let results_Array = new Array(parseInt(request_POST_JSON.length, 10)).fill(String("undefined"));
                        let i_await = parseInt(0, 10);  // 記錄 MariaDB 資料庫返回結果時 for 循環的次數;
                        // 逐條數據寫入 MariaDB 資料庫，拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）的名稱（Key）和取值（Value）字符串;
                        for (let i = 0; i < request_POST_JSON.length; i++) {
                            // console.log(request_POST_JSON[i]);

                            if (typeof (request_POST_JSON[i]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object object]' && !(request_POST_JSON[i].length)) {

                                // 讀取 request_POST_JSON[i] 裏關鍵字（Key）爲「"collection_name"」的鍵值對的值（Value）;
                                if (request_POST_JSON[i].hasOwnProperty("collection_name")) {
                                    // console.log(request_POST_JSON[i]["collection_name"]);
                                    if (!((typeof (request_POST_JSON[i]["collection_name"]) === 'undefined') || ((typeof (request_POST_JSON[i]["collection_name"]) === 'object') && (request_POST_JSON[i]["collection_name"] === null)))) {
                                        if (Object.prototype.toString.call(request_POST_JSON[i]["collection_name"]).toLowerCase() === '[object string]') {
                                            if ((String(request_POST_JSON[i]["collection_name"]).length > 0) && (request_POST_JSON[i]["collection_name"] !== "null") && (request_POST_JSON[i]["collection_name"] !== "NULL") && (request_POST_JSON[i]["collection_name"] !== "Null") && (request_POST_JSON[i]["collection_name"] !== "none") && (request_POST_JSON[i]["collection_name"] !== "NONE") && (request_POST_JSON[i]["collection_name"] !== "None")) {
                                                dbTableName = String(request_POST_JSON[i]["collection_name"]);
                                            };
                                        };
                                    };
                                };
                                // 讀取 request_POST_JSON[i] 裏關鍵字（Key）爲「"table_name"」的鍵值對的值（Value）;
                                if (request_POST_JSON[i].hasOwnProperty("table_name")) {
                                    // console.log(request_POST_JSON[i]["table_name"]);
                                    if (!((typeof (request_POST_JSON[i]["table_name"]) === 'undefined') || ((typeof (request_POST_JSON[i]["table_name"]) === 'object') && (request_POST_JSON[i]["table_name"] === null)))) {
                                        if (Object.prototype.toString.call(request_POST_JSON[i]["table_name"]).toLowerCase() === '[object string]') {
                                            if ((String(request_POST_JSON[i]["table_name"]).length > 0) && (request_POST_JSON[i]["table_name"] !== "null") && (request_POST_JSON[i]["table_name"] !== "NULL") && (request_POST_JSON[i]["table_name"] !== "Null") && (request_POST_JSON[i]["table_name"] !== "none") && (request_POST_JSON[i]["table_name"] !== "NONE") && (request_POST_JSON[i]["table_name"] !== "None")) {
                                                dbTableName = String(request_POST_JSON[i]["table_name"]);
                                            };
                                        };
                                    };
                                };
                                ...
                                if ((typeof (dbName) === 'undefined') || ((typeof (dbName) === 'object') && (dbName === null)) || ((Object.prototype.toString.call(dbName).toLowerCase() === '[object string]') && (dbName === ""))) {
                                    ...
                                    if ((typeof (dbTableName) === 'undefined') || ((typeof (dbTableName) === 'object') && (dbTableName === null)) || ((Object.prototype.toString.call(dbTableName).toLowerCase() === '[object string]') && (dbTableName === ""))) {
                                        ...
                                        console.log("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Database and Table/Collection name requested empty.");

                                        // results_Array.push(String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Database and Table/Collection name requested empty."));
                                        results_Array[i] = String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Database and Table/Collection name requested empty.");
                                        // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String("Database and Table/Collection name requested empty.");
                                        // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        // if (callback) { callback(response_body_String, null); };
                                        ...
                                        i_await = parseInt((parseInt(i_await, 10) + parseInt(1, 10)), 10);
                                        // 向回調函數返回響應值;
                                        if (parseInt(i_await, 10) === parseInt(request_POST_JSON.length, 10)) {
                                            // response_data_JSON["Database_say"] = String(JSON.stringify(results_Array));
                                            response_data_JSON["Database_say"] = results_Array;
                                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                            if (callback) { callback(null, response_body_String); };

                                            results_Array = [];  // = null;  // 歸零復位回收内存;
                                            selectFieldName = [];  // = null;  // 歸零復位回收内存;
                                            request_POST_JSON = {};  // = null;  // 歸零復位回收内存;
                                        };

                                        continue;  // 跳過當前 for 循環繼續執行下一輪;
                                        // break;  // 完全跳出當前 for 循環;
                                        // return response_body_String;

                                    } else {

                                        console.log("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Database name requested empty.");

                                        // results_Array.push(String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Database name requested empty."));
                                        results_Array[i] = String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Database name requested empty.");
                                        // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String("Database name requested empty.");
                                        // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        // if (callback) { callback(response_body_String, null); };
                                    ...
                                    if ((typeof (dbTableName) === 'undefined') || ((typeof (dbTableName) === 'object') && (dbTableName === null)) || ((Object.prototype.toString.call(dbTableName).toLowerCase() === '[object string]') && (dbTableName === ""))) {
                                        ...
                                        console.log("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Table/Collection name requested empty.");

                                        // results_Array.push(String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Table/Collection name requested empty."));
                                        results_Array[i] = String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Table/Collection name requested empty.");
                                        // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String("Table/Collection name requested empty.");
                                        // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        // if (callback) { callback(response_body_String, null); };
                                ...
                                // 拼接用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                                SQL_String = "";  // 初始化清零;
                                // SQL_String = "INSERT INTO \`Collection1\` (\`ReplicationID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`) VALUES (\"001\",\"111\",\"111111\",\"111.111\",\"111111.111111\",\"aaa\",\"aaaaaa\",\"2021-1-1 11:01:01\",\"2021-1-1\",\"11:01:01\",\"1.11\",\"\"),(\"002\",\"222\",\"222222\",\"222.222\",\"222222.222222\",\"bbb\",\"bbbbbb\",\"2022-2-2 12:02:02\",\"2022-2-2\",\"12:02:02\",\"2.22\",\"\");";
                                if (!((JSON.stringify(request_POST_JSON[i]) === '{}') || ((Object.keys(request_POST_JSON[i]).length === 1) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") || request_POST_JSON[i].hasOwnProperty("table_name") || request_POST_JSON[i].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i]).length === 2) && ((request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name")) || (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("collection_name")) || (request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i]).length === 3) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name"))))) {

                                    // 拼接 SQL 語句字符串待操作數據表（TABLE）名字符串;
                                    ...
                                    // 拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）名稱（Key）字符串;
                                    for (let key_POST in request_POST_JSON[i]) {
                                        // console.log(key_POST);
                                        // console.log(request_POST_JSON[i][key_POST]);
                                        if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                            SQL_String = SQL_String + "\`";
                                            SQL_String = SQL_String + String(key_POST);
                                            SQL_String = SQL_String + "\`";
                                            SQL_String = SQL_String + ",";
                                    ...
                                    // 拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）的取值（Value）字符串;
                                    for (let key_POST in request_POST_JSON[i]) {
                                        // console.log(key_POST);
                                        // console.log(request_POST_JSON[i][key_POST]);
                                        if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                            if (typeof (request_POST_JSON[i][key_POST]) === 'undefined') {
                                                SQL_String = SQL_String + '\"';
                                                // SQL_String = SQL_String + String("undefined");
                                                SQL_String = SQL_String + '\"';
                                                SQL_String = SQL_String + ",";
                                            } else if ((typeof (request_POST_JSON[i][key_POST]) === 'object') && (request_POST_JSON[i][key_POST] === null)) {
                                                SQL_String = SQL_String + '\"';
                                                // SQL_String = SQL_String + String("null");
                                                SQL_String = SQL_String + '\"';
                                                SQL_String = SQL_String + ",";
                                            } else if (Object.prototype.toString.call(request_POST_JSON[i][key_POST]).toLowerCase() === '[object string]') {
                                                if ((request_POST_JSON[i][key_POST] === "") || (request_POST_JSON[i][key_POST] === "\"\"") || (request_POST_JSON[i][key_POST] === "\'\'") || (request_POST_JSON[i][key_POST] === "\`\`")) {
                                                    SQL_String = SQL_String + '\"';
                                                    ...
                                                    SQL_String = SQL_String + '\"';
                                                    SQL_String = SQL_String + String(request_POST_JSON[i][key_POST]);
                                                    SQL_String = SQL_String + '\"';
                                                    SQL_String = SQL_String + ",";
                                                };
                                            } else {
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("INSERT INTO " + "\`" + String(dbTableName) + "\`" + " (").length, 10))) {
                                        ...
                                        MariaDB_Client.query(
                                            ...
                                            function(
                                                ...
                                                if (error) {
                                                    ...
                                                    // results_Array.push(String(error.message));
                                                    for (let j = 0; j < results_Array.length; j++) {
                                                        if (results_Array[j] === 'undefined') {
                                                            results_Array[j] = String(error.message);
                                                            break;
                                                        };
                                                    };
                                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                                ...
                                                if (results) {
                                                    ...
                                                    // response_body_String = JSON.stringify(results);
                                                    // results_Array.push(String(results.insertId));
                                                    for (let j = 0; j < results_Array.length; j++) {
                                                        if (results_Array[j] === 'undefined') {
                                                            results_Array[j] = String(results.insertId);
                                                            break;
                                                        };
                                                    };
                                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        ...
                                        console.log("Database error.");
                                        // results_Array.push(String("Database error."));
                                        results_Array[i] = String("Database error.");
                                        // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        // if (callback) { callback(response_body_String, null); };
                                    ...
                                    console.log("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Input request_POST_JSON(Schema Field name or type) empty.");

                                    // results_Array.push(String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Input request_POST_JSON(Schema Field name or type) empty."));
                                    results_Array[i] = String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Input request_POST_JSON(Schema Field name or type) empty.");
                                    // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String("Input request_POST_JSON(Schema Field name or type) empty.");
                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                    // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                    // if (callback) { callback(response_body_String, null); };
                                ...
                                console.log("error, Request post body data unrecognizable.");
                                // results_Array.push(String("error, Request post body data unrecognizable."));
                                results_Array[i] = String("error, Request post body data unrecognizable.");
                                // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                // if (callback) { callback(response_body_String, null); };
                    ...
                    } else if (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length)) {
                        ...
                        if (!((JSON.stringify(request_POST_JSON) === '{}') || ((Object.keys(request_POST_JSON).length === 1) && (request_POST_JSON.hasOwnProperty("selectFieldName") || request_POST_JSON.hasOwnProperty("table_name") || request_POST_JSON.hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON).length === 2) && ((request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name")) || (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("collection_name")) || (request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON).length === 3) && (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name"))))) {
                            ...
                            // 拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）名稱字符串;
                            for (let key_POST in request_POST_JSON) {
                                // console.log(key_POST);
                                // console.log(request_POST_JSON[key_POST]);
                                if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                            ...
                            // 拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）的取值字符串;
                            for (let key_POST in request_POST_JSON) {
                                // console.log(key_POST);
                                // console.log(request_POST_JSON[key_POST]);
                                if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                    if (typeof (request_POST_JSON[key_POST]) === 'undefined') {
                                        SQL_String = SQL_String + '\"';
                                        SQL_String = SQL_String + String("undefined");
                                        SQL_String = SQL_String + '\"';
                                        SQL_String = SQL_String + ",";
                                    } else if ((typeof (request_POST_JSON[key_POST]) === 'object') && (request_POST_JSON[key_POST] === null)) {
                                        SQL_String = SQL_String + '\"';
                                        SQL_String = SQL_String + String("null");
                                        SQL_String = SQL_String + '\"';
                                        SQL_String = SQL_String + ",";
                                    } else if (Object.prototype.toString.call(request_POST_JSON[key_POST]).toLowerCase() === '[object string]') {
                                        if ((request_POST_JSON[key_POST] === "") || (request_POST_JSON[key_POST] === "\"\"") || (request_POST_JSON[key_POST] === "\'\'") || (request_POST_JSON[key_POST] === "\`\`")) {
                                            SQL_String = SQL_String + '\"';
                                            SQL_String = SQL_String + String("");
                                            SQL_String = SQL_String + '\"';
                                            SQL_String = SQL_String + ",";
                                ...
                                MariaDB_Client.query(
                                    ...
                                    function(
                                        ...
                                        if (results) {
                                            ...
                                            // response_body_String = JSON.stringify(results);
                                            response_data_JSON["Database_say"] = String(results.insertId);
                                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                            if (callback) { callback(null, response_body_String); };
                        ...
                        console.log("error, Request post body data unrecognizable.");
                        response_data_JSON["Database_say"] = "error, Request post body data unrecognizable.";
                        response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                        // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                        if (callback) { callback(response_body_String, null); };
                    ...
                    console.log("error, Request post body data is empty.");
                    response_data_JSON["Database_say"] = "error, Request post body data is empty.";
                    response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                    // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                    if (callback) { callback(response_body_String, null); };
        ...
        case "/insertMany": {
        // 向 MariaDB 數據庫伺服器裏指定數據庫裏的指定集合（表格）中插入數據;
        // URL = "/insertMany?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { "Key-field-1" : "Value-field-1", "Key-field-2" : "Value-field-2", ... }
        // request_POST_JSON = [{ "Key-field-1" : "Value-field-1", "Key-field-2" : "Value-field-2", ... }, { "Key-field-1" : "Value-field-1", "Key-field-2" : "Value-field-2", ... }, ... ]
        // request_POST_String = "{\"IntegerShort\":\"111\",\"IntegerLong\":\"111111\",\"FloatSingle\":\"111.111\",\"FloatDouble\":\"111111.111111\",\"TextShort\":\"aaa\",\"TextLong\":\"aaaaaa\",\"DateTime\":\"2021-1-1 11:01:01\",\"Date\":\"2021-1-1\",\"Time\":\"11:01:01\",\"Currency\":\"1.11\",\"Image\":\"\\\"\\\"\"}"
        // request_POST_String = "[{\"IntegerShort\":\"111\",\"IntegerLong\":\"111111\",\"FloatSingle\":\"111.111\",\"FloatDouble\":\"111111.111111\",\"TextShort\":\"aaa\",\"TextLong\":\"aaaaaa\",\"DateTime\":\"2021-1-1 11:01:01\",\"Date\":\"2021-1-1\",\"Time\":\"11:01:01\",\"Currency\":\"1.11\",\"Image\":\"\\\"\\\"\"},{\"IntegerShort\":\"222\",\"IntegerLong\":\"222222\",\"FloatSingle\":\"222.222\",\"FloatDouble\":\"222222.222222\",\"TextShort\":\"bbb\",\"TextLong\":\"bbbbbb\",\"DateTime\":\"2022-2-2 12:02:02\",\"Date\":\"2022-2-2\",\"Time\":\"12:02:02\",\"Currency\":\"2.22\",\"Image\":\"\\\"\\\"\"}]";
            ...
            if (MariaDB_Client !== null) {
                ...
                if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]' && request_POST_JSON.length > 0 && (typeof (request_POST_JSON[0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[0]).toLowerCase() === '[object object]' && !(request_POST_JSON[0].length) && JSON.stringify(request_POST_JSON[0]) !== '{}')) || (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length) && JSON.stringify(request_POST_JSON) !== '{}')) {
                    ...
                    if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]') && (request_POST_JSON.length > 0)) {
                        ...
                        if (typeof (request_POST_JSON[0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[0]).toLowerCase() === '[object object]' && !(request_POST_JSON[0].length)) {

                            // 讀取 request_POST_JSON[0] 裏關鍵字（Key）爲「"collection_name"」的鍵值對的值（Value）;
                            if (request_POST_JSON[0].hasOwnProperty("collection_name")) {
                                // console.log(request_POST_JSON[0]["collection_name"]);
                                if (!((typeof (request_POST_JSON[0]["collection_name"]) === 'undefined') || ((typeof (request_POST_JSON[0]["collection_name"]) === 'object') && (request_POST_JSON[0]["collection_name"] === null)))) {
                                    if (Object.prototype.toString.call(request_POST_JSON[0]["collection_name"]).toLowerCase() === '[object string]') {
                                        if ((String(request_POST_JSON[0]["collection_name"]).length > 0) && (request_POST_JSON[0]["collection_name"] !== "null") && (request_POST_JSON[0]["collection_name"] !== "NULL") && (request_POST_JSON[0]["collection_name"] !== "Null") && (request_POST_JSON[0]["collection_name"] !== "none") && (request_POST_JSON[0]["collection_name"] !== "NONE") && (request_POST_JSON[0]["collection_name"] !== "None")) {
                                            dbTableName = String(request_POST_JSON[0]["collection_name"]);
                                        };
                                    };
                                };
                            };
                            // 讀取 request_POST_JSON[0] 裏關鍵字（Key）爲「"table_name"」的鍵值對的值（Value）;
                            if (request_POST_JSON[0].hasOwnProperty("table_name")) {
                                // console.log(request_POST_JSON[0]["table_name"]);
                                if (!((typeof (request_POST_JSON[0]["table_name"]) === 'undefined') || ((typeof (request_POST_JSON[0]["table_name"]) === 'object') && (request_POST_JSON[0]["table_name"] === null)))) {
                                    if (Object.prototype.toString.call(request_POST_JSON[0]["table_name"]).toLowerCase() === '[object string]') {
                                        if ((String(request_POST_JSON[0]["table_name"]).length > 0) && (request_POST_JSON[0]["table_name"] !== "null") && (request_POST_JSON[0]["table_name"] !== "NULL") && (request_POST_JSON[0]["table_name"] !== "Null") && (request_POST_JSON[0]["table_name"] !== "none") && (request_POST_JSON[0]["table_name"] !== "NONE") && (request_POST_JSON[0]["table_name"] !== "None")) {
                                            dbTableName = String(request_POST_JSON[0]["table_name"]);
                                        };
                                    };
                                };
                            };
                            ...
                            SQL_String = "";  // 初始化清零;
                            // SQL_String = "INSERT INTO \`Collection1\` (\`ReplicationID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`) VALUES (\"001\",\"111\",\"111111\",\"111.111\",\"111111.111111\",\"aaa\",\"aaaaaa\",\"2021-1-1 11:01:01\",\"2021-1-1\",\"11:01:01\",\"1.11\",\"\"),(\"002\",\"222\",\"222222\",\"222.222\",\"222222.222222\",\"bbb\",\"bbbbbb\",\"2022-2-2 12:02:02\",\"2022-2-2\",\"12:02:02\",\"2.22\",\"\");";
                            if (!((JSON.stringify(request_POST_JSON[0]) === '{}') || ((Object.keys(request_POST_JSON[0]).length === 1) && (request_POST_JSON[0].hasOwnProperty("selectFieldName") || request_POST_JSON[0].hasOwnProperty("table_name") || request_POST_JSON[0].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[0]).length === 2) && ((request_POST_JSON[0].hasOwnProperty("selectFieldName") && request_POST_JSON[0].hasOwnProperty("table_name")) || (request_POST_JSON[0].hasOwnProperty("selectFieldName") && request_POST_JSON[0].hasOwnProperty("collection_name")) || (request_POST_JSON[0].hasOwnProperty("table_name") && request_POST_JSON[0].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[0]).length === 3) && (request_POST_JSON[0].hasOwnProperty("selectFieldName") && request_POST_JSON[0].hasOwnProperty("table_name") && request_POST_JSON[0].hasOwnProperty("collection_name"))))) {
                                ...
                                // 拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）名稱字符串;
                                for (let key_POST in request_POST_JSON[0]) {
                                    // console.log(key_POST);
                                    // console.log(request_POST_JSON[0][key_POST]);
                                    if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                        SQL_String = SQL_String + "\`";
                                        SQL_String = SQL_String + String(key_POST);
                                        SQL_String = SQL_String + "\`";
                                ...
                                // 拼接字段（Filed）名稱（Key）結尾的右括弧（")"）字符;
                                SQL_String = SQL_String + ")";

                                // 拼接字段（Filed）取值（Value）起始的保留字（"VALUES"）字符;
                                SQL_String = SQL_String + " VALUES ";
                                ...
                                if (String(SQL_String).length > 0) {
                                    ...
                                    // // 判斷 SQL 語句字符串末位是否爲「"),"」字符，若爲「"),"」字符，則將其替換爲「");"」字符，作爲 SQL 語句的結尾;
                                    // if (String(SQL_String.slice(-2)) === "),") {
                                    //     // 刪除 SQL 語句字符串末位多餘的「),」字符;
                                    //     SQL_String = String(SQL_String.slice(0, -2));
                                    //     // 拼接 SQL 語句字符串末位結束符分號（;）字符;
                                    //     SQL_String = SQL_String + ");";
                                    // };
                                };
                                ...
                                console.log("[ " + String(parseInt((parseInt(0, 10) + parseInt(1, 10)), 10)) + " ] : " + "Input request_POST_JSON(Schema Field name or type) empty.");
                                response_data_JSON["Database_say"] = "[ " + String(parseInt((parseInt(0, 10) + parseInt(1, 10)), 10)) + " ] : " + "Input request_POST_JSON(Schema Field name or type) empty.";
                                response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                if (callback) { callback(null, response_body_String); };
                    ...
                    // 檢查判斷 SQL 語句字符串末位是否爲「");"」字符，若否爲「");"」字符，則判斷爲 SQL 語句拼接錯誤，中止後續執行跳出函數;
                    if (String(SQL_String).length > 0) {
                        // 判斷 SQL 語句字符串末位是否爲「");"」字符，若否爲「");"」字符，則判斷爲 SQL 語句拼接錯誤，中止後續執行跳出函數;
                        if (String(SQL_String.slice(-2)) !== ");") {
                            console.log("Input request_POST_JSON(Schema Field name or type) error.");
                            console.log(SQL_String);
                            response_data_JSON["Database_say"] = "Input request_POST_JSON(Schema Field name or type) error.";
                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                            if (callback) { callback(response_body_String, null); };
                        ...
                        // // 判斷 SQL 語句字符串末位是否爲「"VALUES"」字符，若爲「"VALUES"」字符，則判斷爲 SQL 語句拼接錯誤，中止後續執行跳出函數;
                        // if ((String(SQL_String.slice(-6)) === "VALUES") || (String(SQL_String.slice(-7)) === "VALUES ")) {
                        //     console.log("Input request_POST_JSON(Schema Field name or type) empty.");
                        //     console.log(SQL_String);
                        //     response_data_JSON["Database_say"] = "Input request_POST_JSON(Schema Field name or type) empty.";
                        //     response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
        ...
        case "/deleteOne": {
        // 從 MariaDB 數據庫伺服器裏指定數據庫裏指定的集合（表格）中刪除數據;
        // URL = "/deleteOne?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { WHERE }
        // request_POST_JSON = [{ WHERE-1 }, { WHERE-1 }, ... ]
        // request_POST_String = "{\"AutoID\":\"= 1\",\"IntegerShort\":\"= 111\",\"FloatDouble\":\"= 111111.111111\",\"TextShort\":\"= \\\"aaa\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}";
        // request_POST_String = "[{\"AutoID\":\"= 1\",\"IntegerShort\":\"= 111\",\"FloatDouble\":\"= 111111.111111\",\"TextShort\":\"= \\\"aaa\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"},{\"AutoID\":\"= 2\",\"IntegerShort\":\"= 222\",\"FloatDouble\":\"= 222222.222222\",\"TextShort\":\"= \\\"bbb\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}]";
        // DELETE FROM `Collection1` WHERE ((`AutoID` = 1) AND (`IntegerShort` = 111) AND (`FloatDouble` = 111111.111111) AND (`TextShort` = "aaa") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07') AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        ...
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            if (MariaDB_Client !== null) {
                ...
                // 拼接用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                SQL_String = "";  // 初始化清零;
                // SQL_String = "DELETE FROM `Collection1` WHERE ((`AutoID` = 1) AND (`IntegerShort` = 111) AND (`FloatDouble` = 111111.111111) AND (`TextShort` = "aaa") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07') AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));";
                // SQL_String = "DELETE FROM `Collection1`;";
                if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]' && request_POST_JSON.length > 0 && (typeof (request_POST_JSON[0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[0]).toLowerCase() === '[object object]' && !(request_POST_JSON[0].length) && JSON.stringify(request_POST_JSON[0]) !== '{}')) || (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length) && JSON.stringify(request_POST_JSON) !== '{}')) {

                    // 判斷傳入變量：request_POST_JSON 的數據類型，數組（Array）或 JSON 類型;
                    if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]') && (request_POST_JSON.length > 0)) {
                        ...
                        for (let i = 0; i < request_POST_JSON.length; i++) {
                            ...
                            if (typeof (request_POST_JSON[i]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object object]' && !(request_POST_JSON[i].length)) {
                                ...
                                if (!((JSON.stringify(request_POST_JSON[i]) === '{}') || ((Object.keys(request_POST_JSON[i]).length === 1) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") || request_POST_JSON[i].hasOwnProperty("table_name") || request_POST_JSON[i].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i]).length === 2) && ((request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name")) || (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("collection_name")) || (request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i]).length === 3) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name"))))) {
                                    ...
                                    // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                                    ...
                                    for (let key_POST in request_POST_JSON[i]) {
                                        ...
                                        if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                            ...
                                            // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）名稱（Key）字符串;
                                            SQL_String = SQL_String + "(";
                                            SQL_String = SQL_String + "\`";
                                            SQL_String = SQL_String + String(key_POST);
                                            SQL_String = SQL_String + "\`";
                                            ...
                                            // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）的取值（Value）字符串;
                                            if (typeof (request_POST_JSON[i][key_POST]) === 'undefined') {
                                                // SQL_String = SQL_String + '\"';
                                                // SQL_String = SQL_String + String("undefined");
                                                // SQL_String = SQL_String + '\"';
                                            ...
                                            // 拼接 SQL 語句字符串待檢索條件之間連接運算符號（AND/OR/NOT）;
                                            SQL_String = SQL_String + " AND ";
                                        };
                                    };

                                    // 刪除 SQL 語句字符串末位多餘的檢索條件之間連接運算符號（AND/OR/NOT）;
                                    if (String(SQL_String).length > 0) {
                                        if (String(SQL_String.slice(-5)) === " AND ") {
                                            SQL_String = String(SQL_String.slice(0, -5));
                                        };
                                    };
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("DELETE FROM " + "\`" + String(dbTableName) + "\`" + " WHERE (").length, 10))) {
                                        ...
                                        MariaDB_Client.query(
                                            ...
                                            function(
                                                ...
                                                if (results) {
                                                    ...
                                                    // response_body_String = JSON.stringify(results);
                                                    // results_Array.push(String(results.affectedRows));
                                                    for (let j = 0; j < results_Array.length; j++) {
                                                        if (results_Array[j] === 'undefined') {
                                                            results_Array[j] = String(results.affectedRows);
                                                            break;
                                                        };
                                                    };
                                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                    ...
                                    // console.log("[" + String((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "Request post Body is empty.");
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("DELETE FROM ").length, 10))) {
                                ...
                                console.log("[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "error, Request post body data unrecognizable.");
                        ...
                        if (!((JSON.stringify(request_POST_JSON) === '{}') || ((Object.keys(request_POST_JSON).length === 1) && (request_POST_JSON.hasOwnProperty("selectFieldName") || request_POST_JSON.hasOwnProperty("table_name") || request_POST_JSON.hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON).length === 2) && ((request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name")) || (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("collection_name")) || (request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON).length === 3) && (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name"))))) {
                                ...
                                MariaDB_Client.query(
                                    ...
                                    function(
                                        ...
                                        if (results) {
                                            ...
                                            // response_body_String = JSON.stringify(results);
                                            response_data_JSON["Database_say"] = String(results.affectedRows);
                                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                            if (callback) { callback(null, response_body_String); };
                            ...
                            // console.log("Request post Body is empty.");
        ...
        case "/deleteMany": {
        // 從 MariaDB 數據庫伺服器裏指定數據庫裏指定的集合（表格）中刪除數據;
        // URL = "/deleteMany?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { WHERE }
        // request_POST_JSON = [{ WHERE-1 }, { WHERE-1 }, ... ]
        // request_POST_String = "{\"AutoID\":\"= 1\",\"IntegerShort\":\"= 111\",\"FloatDouble\":\"= 111111.111111\",\"TextShort\":\"= \\\"aaa\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}";
        // request_POST_String = "[{\"AutoID\":\"= 1\",\"IntegerShort\":\"= 111\",\"FloatDouble\":\"= 111111.111111\",\"TextShort\":\"= \\\"aaa\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"},{\"AutoID\":\"= 2\",\"IntegerShort\":\"= 222\",\"FloatDouble\":\"= 222222.222222\",\"TextShort\":\"= \\\"bbb\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}]";
                        ...
                        for (let i = 0; i < request_POST_JSON.length; i++) {
                            ...
                            if (typeof (request_POST_JSON[i]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object object]' && !(request_POST_JSON[i].length)) {
                                ...
                                if (!((JSON.stringify(request_POST_JSON[i]) === '{}') || ((Object.keys(request_POST_JSON[i]).length === 1) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") || request_POST_JSON[i].hasOwnProperty("table_name") || request_POST_JSON[i].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i]).length === 2) && ((request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name")) || (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("collection_name")) || (request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i]).length === 3) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name"))))) {
                                    ...
                                    // console.log("[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "Request post Body is empty.");
        ...
        case "/updateOne": {
        // 更新 MariaDB 資料庫伺服器裏指定資料庫裏指定集合（表格）中的指定數據;
        // URL = "/updateOne?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = [{ SET }, { WHERE }]
        // request_POST_String = "[{\"IntegerShort\":\"2111\",\"IntegerLong\":\"2111111\",\"FloatSingle\":\"2111.111\",\"FloatDouble\":\"2111111.111111\",\"TextShort\":\"2aaa\",\"TextLong\":\"2aaaaaa\",\"DateTime\":\"3021-11-11 23:23:23\",\"Date\":\"3021-11-11\",\"Time\":\"23:23:23\",\"Currency\":\"9.99\"},{\"AutoID\":\"= 1\",\"IntegerShort\":\"= 111\",\"FloatDouble\":\"= 111111.111111\",\"TextShort\":\"= \\\"aaa\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}]";
        // UPDATE `Collection1` SET `IntegerShort`="2111",`IntegerLong`="2111111",`FloatSingle`="2111.111",`FloatDouble`="2111111.111111",`TextShort`="2aaa",`TextLong`="2aaaaaa",`DateTime`="3021-11-11 23:23:23",`Date`="3021-11-11",`Time`="23:23:23",`Currency`="9.99" WHERE ((`AutoID` = 1) AND (`IntegerShort` = 111) AND (`FloatDouble` = 111111.111111) AND (`TextShort` = "aaa") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07') AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        // UPDATE `Collection1` SET `IntegerShort`="2111",`IntegerLong`="2111111",`FloatSingle`="2111.111",`FloatDouble`="2111111.111111",`TextShort`="2aaa",`TextLong`="2aaaaaa",`DateTime`="3021-11-11 23:23:23",`Date`="3021-11-11",`Time`="23:23:23",`Currency`="9.99";
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            if (MariaDB_Client !== null) {
                ...
                if (Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]' && request_POST_JSON.length > 0) {
                    ...
                    // 拼接用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                    SQL_String = "";  // 初始化清零;
                    // SQL_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\";";
                    // SQL_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\" WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \"aaa\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\')  AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
                    // 單次模式，只容納一組檢索條件執行一次更新;
                    // [{ SET }, { WHERE }]
                    if (typeof (request_POST_JSON[0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[0]).toLowerCase() === '[object object]' && !(request_POST_JSON[0].length)) {
                        ...
                        if (!((JSON.stringify(request_POST_JSON[0]) === '{}') || ((Object.keys(request_POST_JSON[0]).length === 1) && (request_POST_JSON[0].hasOwnProperty("selectFieldName") || request_POST_JSON[0].hasOwnProperty("table_name") || request_POST_JSON[0].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[0]).length === 2) && ((request_POST_JSON[0].hasOwnProperty("selectFieldName") && request_POST_JSON[0].hasOwnProperty("table_name")) || (request_POST_JSON[0].hasOwnProperty("selectFieldName") && request_POST_JSON[0].hasOwnProperty("collection_name")) || (request_POST_JSON[0].hasOwnProperty("table_name") && request_POST_JSON[0].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[0]).length === 3) && (request_POST_JSON[0].hasOwnProperty("selectFieldName") && request_POST_JSON[0].hasOwnProperty("table_name") && request_POST_JSON[0].hasOwnProperty("collection_name"))))) {
                            ...
                            // 拼接 SQL 語句字符串待改寫字段（SET），數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                            ...
                            for (let key_POST in request_POST_JSON[0]) {
                                ...
                                if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                    ...
                                    // 拼接 SQL 語句字符串待改寫字段名稱（Key），數據表（Table/Collection）的字段（field）名稱（Key）字符串;
                                    SQL_String = SQL_String + "\`";
                                    SQL_String = SQL_String + String(key_POST);
                                    SQL_String = SQL_String + "\`";
                                    SQL_String = SQL_String + "=";

                                    // 拼接 SQL 語句字符串待改寫字段值（Value），數據表（Table/Collection）的字段（field）的取值（Value）字符串;
                                    if (typeof (request_POST_JSON[0][key_POST]) === 'undefined') {
                                        SQL_String = SQL_String + '\"';
                                        // SQL_String = SQL_String + String("undefined");
                                        SQL_String = SQL_String + '\"';
                                    } else if ((typeof (request_POST_JSON[0][key_POST]) === 'object') && (request_POST_JSON[0][key_POST] === null)) {
                                        SQL_String = SQL_String + '\"';
                                        // SQL_String = SQL_String + String("null");
                                        SQL_String = SQL_String + '\"';
                                    } else if (Object.prototype.toString.call(request_POST_JSON[0][key_POST]).toLowerCase() === '[object string]') {
                                        if ((request_POST_JSON[0][key_POST] === "") || (request_POST_JSON[0][key_POST] === "\"\"") || (request_POST_JSON[0][key_POST] === "\'\'") || (request_POST_JSON[0][key_POST] === "\`\`")) {
                                            SQL_String = SQL_String + '\"';
                                            SQL_String = SQL_String + String("");
                                            SQL_String = SQL_String + '\"';
                                        } else {
                                            SQL_String = SQL_String + '\"';
                                            SQL_String = SQL_String + String(request_POST_JSON[0][key_POST]);
                                            SQL_String = SQL_String + '\"';
                                        };
                                    } else {
                                        SQL_String = SQL_String + '\"';
                            ...
                            // 拼接 SQL 語句字符串待檢索條件（WHERE），數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                            if (request_POST_JSON.length === 1) {
                                ...
                                // 向 MariaDB 資料庫伺服器發送 SQL 語句字符串執行並返回結果;
                                if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("UPDATE " + "\`" + String(dbTableName) + "\`" + " SET ").length, 10))) {
                            ...
                            } else if (request_POST_JSON.length >= 2) {

                                if ((typeof (request_POST_JSON[1]) === 'undefined') || ((typeof (request_POST_JSON[1]) === 'object') && (request_POST_JSON[1] === null)) || ((Object.prototype.toString.call(request_POST_JSON[1]).toLowerCase() === '[object string]') && (request_POST_JSON[1] === ""))) {
                                ...
                                } else if (typeof (request_POST_JSON[1]) === 'object' && Object.prototype.toString.call(request_POST_JSON[1]).toLowerCase() === '[object object]' && !(request_POST_JSON[1].length)) {

                                    if ((JSON.stringify(request_POST_JSON[1]) === '{}') || ((Object.keys(request_POST_JSON[1]).length === 1) && (request_POST_JSON[1].hasOwnProperty("selectFieldName") || request_POST_JSON[1].hasOwnProperty("table_name") || request_POST_JSON[1].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[1]).length === 2) && ((request_POST_JSON[1].hasOwnProperty("selectFieldName") && request_POST_JSON[1].hasOwnProperty("table_name")) || (request_POST_JSON[1].hasOwnProperty("selectFieldName") && request_POST_JSON[1].hasOwnProperty("collection_name")) || (request_POST_JSON[1].hasOwnProperty("table_name") && request_POST_JSON[1].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[1]).length === 3) && (request_POST_JSON[1].hasOwnProperty("selectFieldName") && request_POST_JSON[1].hasOwnProperty("table_name") && request_POST_JSON[1].hasOwnProperty("collection_name")))) {
                                        ...
                                        // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                                        SQL_String = SQL_String + " WHERE (";
                                        for (let key_POST in request_POST_JSON[1]) {
                                            // console.log(key_POST);
                                            // console.log(request_POST_JSON[1][key_POST]);
                                            ...
                                            if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                                ...
                                                // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）的取值（Value）字符串;
                                                if (typeof (request_POST_JSON[1][key_POST]) === 'undefined') {
                                                    // SQL_String = SQL_String + '\"';
                                                    // SQL_String = SQL_String + String("undefined");
                                                    // SQL_String = SQL_String + '\"';
                                                    SQL_String = SQL_String + ")";
                                                } else if ((typeof (request_POST_JSON[1][key_POST]) === 'object') && (request_POST_JSON[1][key_POST] === null)) {
                                                    // SQL_String = SQL_String + '\"';
                                                    // SQL_String = SQL_String + String("null");
                                                    // SQL_String = SQL_String + '\"';
                                                    SQL_String = SQL_String + ")";
                                                } else if (Object.prototype.toString.call(request_POST_JSON[1][key_POST]).toLowerCase() === '[object string]') {
                                                    if ((request_POST_JSON[1][key_POST] === "") || (request_POST_JSON[1][key_POST] === "\"\"") || (request_POST_JSON[1][key_POST] === "\'\'") || (request_POST_JSON[1][key_POST] === "\`\`")) {
                                                        // SQL_String = SQL_String + '\"';
                                                        SQL_String = SQL_String + String("");
                                                        // SQL_String = SQL_String + '\"';
                                                        SQL_String = SQL_String + ")";
                                                        ...
                                                        // SQL_String = SQL_String + '\"';
                                                        SQL_String = SQL_String + String(request_POST_JSON[1][key_POST]);
                                                        // SQL_String = SQL_String + '\"';
                                                        SQL_String = SQL_String + ")";
                                                    };
                                                } else {
                            ...
                            console.log("error, Request post body request_POST_JSON update(Schema Field name or type) empty.");

                            response_data_JSON["Database_say"] = "error, Request post body request_POST_JSON update(Schema Field name or type) empty.";
                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                            if (callback) { callback(null, response_body_String); };
                        ...
                        console.log("error, Request post body request_POST_JSON update(Schema Field name or type) unrecognizable.");

                        response_data_JSON["Database_say"] = "error, Request post body request_POST_JSON update(Schema Field name or type) unrecognizable.";
                        response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                        // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                        if (callback) { callback(response_body_String, null); };
        ...
        case "/updateMany": {
        // 更新 MariaDB 資料庫伺服器裏指定資料庫裏指定集合（表格）中的指定數據;
        // URL = "/updateMany?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = [[{ SET-1 }, { WHERE-1 }], [{ SET-2 }, { WHERE-2 }], ... ]
        // request_POST_String = "[[{\"IntegerShort\":\"2111\",\"IntegerLong\":\"2111111\",\"FloatSingle\":\"2111.111\",\"FloatDouble\":\"2111111.111111\",\"TextShort\":\"2aaa\",\"TextLong\":\"2aaaaaa\",\"DateTime\":\"3021-11-11 23:23:23\",\"Date\":\"3021-11-11\",\"Time\":\"23:23:23\",\"Currency\":\"9.99\"},{\"AutoID\":\"= 1\",\"IntegerShort\":\"= 111\",\"FloatDouble\":\"= 111111.111111\",\"TextShort\":\"= \\\"aaa\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}],[],[{\"IntegerShort\":\"3111\",\"IntegerLong\":\"3111111\",\"FloatSingle\":\"3111.111\",\"FloatDouble\":\"3111111.111111\",\"TextShort\":\"3bbb\",\"TextLong\":\"3bbbbbb\",\"DateTime\":\"5021-11-11 23:23:23\",\"Date\":\"5021-11-11\",\"Time\":\"23:23:23\",\"Currency\":\"11.11\"},{\"AutoID\":\"= 2\",\"IntegerShort\":\"= 222\",\"FloatDouble\":\"= 222222.222222\",\"TextShort\":\"= \\\"bbb\\\"\",\"DateTime\":\"BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\'\",\"Date\":\"BETWEEN \'2021-01-01\' AND \'2027-07-07\'\",\"Time\":\"BETWEEN \'11:01:01\' AND \'17:07:07\'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}]]";
        // UPDATE `Collection1` SET `IntegerShort`="2111",`IntegerLong`="2111111",`FloatSingle`="2111.111",`FloatDouble`="2111111.111111",`TextShort`="2aaa",`TextLong`="2aaaaaa",`DateTime`="3021-11-11 23:23:23",`Date`="3021-11-11",`Time`="23:23:23",`Currency`="9.99" WHERE ((`AutoID` = 1) AND (`IntegerShort` = 111) AND (`FloatDouble` = 111111.111111) AND (`TextShort` = "aaa") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07') AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        // UPDATE `Collection1` SET `IntegerShort`="2111",`IntegerLong`="2111111",`FloatSingle`="2111.111",`FloatDouble`="2111111.111111",`TextShort`="2aaa",`TextLong`="2aaaaaa",`DateTime`="3021-11-11 23:23:23",`Date`="3021-11-11",`Time`="23:23:23",`Currency`="9.99";
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            if (MariaDB_Client !== null) {
                ...
                if (Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]' && request_POST_JSON.length > 0) {
                    ...
                    // 批量模式，多組檢索條件次序執行;
                    // [[{ SET-1 }, { WHERE-1 }], [{ SET-2 }, { WHERE-2 }], ... ]
                    let results_Array = new Array(parseInt(request_POST_JSON.length, 10)).fill(String("undefined"));
                    let i_await = parseInt(0, 10);  // 記錄 MariaDB 資料庫返回結果時 for 循環的次數;
                    // 逐條數據寫入 MariaDB 資料庫，拼接 SET 和 WHERE 參數，拼接 SQL 語句字符串待插入數據表（Table/Collection）的字段（field）的名稱（Key）和取值（Value）字符串;
                    for (let i = 0; i < request_POST_JSON.length; i++) {
                        // console.log(request_POST_JSON[i]);
                        ...
                        // SQL_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\";";
                        // SQL_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\" WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \"aaa\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\')  AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
                        if (Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object array]') {

                            if (request_POST_JSON[i].length > 0) {
                                if (typeof (request_POST_JSON[i][0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i][0]).toLowerCase() === '[object object]' && !(request_POST_JSON[i][0].length)) {

                                    // 讀取 request_POST_JSON[i][0] 裏關鍵字（Key）爲「"collection_name"」的鍵值對的值（Value）;
                                    if (request_POST_JSON[i][0].hasOwnProperty("collection_name")) {
                                        // console.log(request_POST_JSON[i][0]["collection_name"]);
                                        if (!((typeof (request_POST_JSON[i][0]["collection_name"]) === 'undefined') || ((typeof (request_POST_JSON[i][0]["collection_name"]) === 'object') && (request_POST_JSON[i][0]["collection_name"] === null)))) {
                                            if (Object.prototype.toString.call(request_POST_JSON[i][0]["collection_name"]).toLowerCase() === '[object string]') {
                                                if ((String(request_POST_JSON[i][0]["collection_name"]).length > 0) && (request_POST_JSON[i][0]["collection_name"] !== "null") && (request_POST_JSON[i][0]["collection_name"] !== "NULL") && (request_POST_JSON[i][0]["collection_name"] !== "Null") && (request_POST_JSON[i][0]["collection_name"] !== "none") && (request_POST_JSON[i][0]["collection_name"] !== "NONE") && (request_POST_JSON[i][0]["collection_name"] !== "None")) {
                                                    dbTableName = String(request_POST_JSON[i][0]["collection_name"]);
                                                };
                                            };
                                        };
                                    };
                                    // 讀取 request_POST_JSON[i][0] 裏關鍵字（Key）爲「"table_name"」的鍵值對的值（Value）;
                                    if (request_POST_JSON[i][0].hasOwnProperty("table_name")) {
                                        // console.log(request_POST_JSON[i][0]["table_name"]);
                                        if (!((typeof (request_POST_JSON[i][0]["table_name"]) === 'undefined') || ((typeof (request_POST_JSON[i][0]["table_name"]) === 'object') && (request_POST_JSON[i][0]["table_name"] === null)))) {
                                            if (Object.prototype.toString.call(request_POST_JSON[i][0]["table_name"]).toLowerCase() === '[object string]') {
                                                if ((String(request_POST_JSON[i][0]["table_name"]).length > 0) && (request_POST_JSON[i][0]["table_name"] !== "null") && (request_POST_JSON[i][0]["table_name"] !== "NULL") && (request_POST_JSON[i][0]["table_name"] !== "Null") && (request_POST_JSON[i][0]["table_name"] !== "none") && (request_POST_JSON[i][0]["table_name"] !== "NONE") && (request_POST_JSON[i][0]["table_name"] !== "None")) {
                                                    dbTableName = String(request_POST_JSON[i][0]["table_name"]);
                                                };
                                            };
                                        };
                                    };
                                    ...
                                    // SQL_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\";";
                                    // SQL_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\" WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \"aaa\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\')  AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
                                    if (!((JSON.stringify(request_POST_JSON[i][0]) === '{}') || ((Object.keys(request_POST_JSON[i][0]).length === 1) && (request_POST_JSON[i][0].hasOwnProperty("selectFieldName") || request_POST_JSON[i][0].hasOwnProperty("table_name") || request_POST_JSON[i][0].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i][0]).length === 2) && ((request_POST_JSON[i][0].hasOwnProperty("selectFieldName") && request_POST_JSON[i][0].hasOwnProperty("table_name")) || (request_POST_JSON[i][0].hasOwnProperty("selectFieldName") && request_POST_JSON[i][0].hasOwnProperty("collection_name")) || (request_POST_JSON[i][0].hasOwnProperty("table_name") && request_POST_JSON[i][0].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i][0]).length === 3) && (request_POST_JSON[i][0].hasOwnProperty("selectFieldName") && request_POST_JSON[i][0].hasOwnProperty("table_name") && request_POST_JSON[i][0].hasOwnProperty("collection_name"))))) {
                                        ...
                                        // 拼接 SQL 語句字符串待改寫字段（SET），數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                                        SQL_String = SQL_String + " SET ";
                                        for (let key_POST in request_POST_JSON[i][0]) {
                                            // console.log(key_POST);
                                            // console.log(request_POST_JSON[i][0][key_POST]);
                                            ...
                                            if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                                ...
                                                // 拼接 SQL 語句字符串待改寫字段值（Value），數據表（Table/Collection）的字段（field）的取值（Value）字符串;
                                                if (typeof (request_POST_JSON[i][0][key_POST]) === 'undefined') {
                                                    SQL_String = SQL_String + '\"';
                                                    // SQL_String = SQL_String + String("undefined");
                                                    SQL_String = SQL_String + '\"';
                                                } else if ((typeof (request_POST_JSON[i][0][key_POST]) === 'object') && (request_POST_JSON[i][0][key_POST] === null)) {
                                                    SQL_String = SQL_String + '\"';
                                                    // SQL_String = SQL_String + String("null");
                                                    SQL_String = SQL_String + '\"';
                                                } else if (Object.prototype.toString.call(request_POST_JSON[i][0][key_POST]).toLowerCase() === '[object string]') {
                                                    if ((request_POST_JSON[i][0][key_POST] === "") || (request_POST_JSON[i][0][key_POST] === "\"\"") || (request_POST_JSON[i][0][key_POST] === "\'\'") || (request_POST_JSON[i][0][key_POST] === "\`\`")) {
                                                        SQL_String = SQL_String + '\"';
                                                        SQL_String = SQL_String + String("");
                                                        SQL_String = SQL_String + '\"';
                                                    } else {
                                                        SQL_String = SQL_String + '\"';
                                                        SQL_String = SQL_String + String(request_POST_JSON[i][0][key_POST]);
                                                        SQL_String = SQL_String + '\"';
                                                    };
                                                } else {
                                                    SQL_String = SQL_String + '\"';
                                        ...
                                        // 拼接 SQL 語句字符串待檢索條件（WHERE），數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                                        if (request_POST_JSON[i].length === 1) {
                                        ...
                                        } else if (request_POST_JSON[i].length >= 2) {

                                            if ((typeof (request_POST_JSON[i][1]) === 'undefined') || ((typeof (request_POST_JSON[i][1]) === 'object') && (request_POST_JSON[i][1] === null)) || ((Object.prototype.toString.call(request_POST_JSON[i][1]).toLowerCase() === '[object string]') && (request_POST_JSON[i][1] === ""))) {
                                            ...
                                            } else if (typeof (request_POST_JSON[i][1]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i][1]).toLowerCase() === '[object object]' && !(request_POST_JSON[i][1].length)) {

                                                if ((JSON.stringify(request_POST_JSON[i][1]) === '{}') || ((Object.keys(request_POST_JSON[i][1]).length === 1) && (request_POST_JSON[i][1].hasOwnProperty("selectFieldName") || request_POST_JSON[i][1].hasOwnProperty("table_name") || request_POST_JSON[i][1].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i][1]).length === 2) && ((request_POST_JSON[i][1].hasOwnProperty("selectFieldName") && request_POST_JSON[i][1].hasOwnProperty("table_name")) || (request_POST_JSON[i][1].hasOwnProperty("selectFieldName") && request_POST_JSON[i][1].hasOwnProperty("collection_name")) || (request_POST_JSON[i][1].hasOwnProperty("table_name") && request_POST_JSON[i][1].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i][1]).length === 3) && (request_POST_JSON[i][1].hasOwnProperty("selectFieldName") && request_POST_JSON[i][1].hasOwnProperty("table_name") && request_POST_JSON[i][1].hasOwnProperty("collection_name")))) {
                                                    ...
                                                    // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）名稱（Key）和取值（Value）字符串;
                                                    SQL_String = SQL_String + " WHERE (";
                                                    for (let key_POST in request_POST_JSON[i][1]) {
                                                        // console.log(key_POST);
                                                        // console.log(request_POST_JSON[i][1][key_POST]);
                                                        ...
                                                        if ((key_POST !== "table_name") && (key_POST !== "collection_name") && (key_POST !== "selectFieldName") && (String(key_POST).length > 0)) {
                                                            ...
                                                            // 拼接 SQL 語句字符串待檢索條件，數據表（Table/Collection）的字段（field）的取值（Value）字符串;
                                                            if (typeof (request_POST_JSON[i][1][key_POST]) === 'undefined') {
                                                                // SQL_String = SQL_String + '\"';
                                                                // SQL_String = SQL_String + String("undefined");
                                                                // SQL_String = SQL_String + '\"';
                                                                SQL_String = SQL_String + ")";
                                                            } else if ((typeof (request_POST_JSON[i][1][key_POST]) === 'object') && (request_POST_JSON[i][1][key_POST] === null)) {
                                                                // SQL_String = SQL_String + '\"';
                                                                // SQL_String = SQL_String + String("null");
                                                                // SQL_String = SQL_String + '\"';
                                                                SQL_String = SQL_String + ")";
                                                            } else if (Object.prototype.toString.call(request_POST_JSON[i][1][key_POST]).toLowerCase() === '[object string]') {
                                                                if ((request_POST_JSON[i][1][key_POST] === "") || (request_POST_JSON[i][1][key_POST] === "\"\"") || (request_POST_JSON[i][1][key_POST] === "\'\'") || (request_POST_JSON[i][1][key_POST] === "\`\`")) {
                                                                    // SQL_String = SQL_String + '\"';
                                                                    SQL_String = SQL_String + String("");
                                                                    // SQL_String = SQL_String + '\"';
                                                                    SQL_String = SQL_String + ")";
                                                                    ...
                                                                    // SQL_String = SQL_String + '\"';
                                                                    SQL_String = SQL_String + String(request_POST_JSON[i][1][key_POST]);
                                                                    // SQL_String = SQL_String + '\"';
                                                                    SQL_String = SQL_String + ")";
                                                                };
                                                            } else {
                                        ...
                                        console.log("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Request post body request_POST_JSON update(Schema Field name or type) empty.");

                                        // results_Array.push(String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Request post body request_POST_JSON update(Schema Field name or type) empty."));
                                        results_Array[i] = String("[ " + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + " ] : " + "Request post body request_POST_JSON update(Schema Field name or type) empty.");
                                        // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String("Request post body request_POST_JSON update(Schema Field name or type) empty.");
                                        // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        // if (callback) { callback(response_body_String, null); };
                            ...
                            console.log("[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "Request post body data unrecognizable.");

                            // results_Array.push(String("[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "Request post body data unrecognizable."));
                            results_Array[i] = String("[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "Request post body data unrecognizable.");
                            // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                            // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                            // if (callback) { callback(response_body_String, null); };
        ...
        case "/find": {
        // 向 MariaDB 資料庫伺服器裏指定資料庫裏的指定集合（表格）中發出檢索指令，並接收返回數據;
        // URL = "/find?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1&selectFieldName=[\"*\",\"AutoID\",\"IntegerShort\",\"FloatSingle\",\"TextShort\",\"DateTime\",\"Currency\",\"Time_Stamp\"]";
        // selectFieldName = [*, "Field-name-1", "Field-name-2", "Field-name-3", ... ]
        // request_POST_JSON = { WHERE }
        // request_POST_JSON = [{ WHERE-1 }, { WHERE-1 }, ... ]
        // request_POST_String = "{\"selectFieldName\":\"\`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Currency\`,\`Time_Stamp\`\",\"AutoID\":\"BETWEEN 1 AND 7\",\"IntegerShort\":\"BETWEEN 111 AND 777\",\"FloatDouble\":\"BETWEEN 111111.111111 AND 777777.777777\",\"TextShort\":\"NOT LIKE \\\"%aa%\\\"\",\"DateTime\":\"BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07'\",\"Date\":\"BETWEEN '2021-01-01' AND '2027-07-07'\",\"Time\":\"BETWEEN '11:01:01' AND '17:07:07'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}";
        // request_POST_String = "[{\"selectFieldName\":\"\`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Currency\`,\`Time_Stamp\`\",\"AutoID\":\"BETWEEN 1 AND 7\",\"IntegerShort\":\"BETWEEN 111 AND 777\",\"FloatDouble\":\"BETWEEN 111111.111111 AND 777777.777777\",\"TextShort\":\"NOT LIKE \\\"%aa%\\\"\",\"DateTime\":\"BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07'\",\"Date\":\"BETWEEN '2021-01-01' AND '2027-07-07'\",\"Time\":\"BETWEEN '11:01:01' AND '17:07:07'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"},{\"selectFieldName\":\"\`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Currency\`,\`Time_Stamp\`\"},{\"selectFieldName\":\"\`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Currency\`,\`Time_Stamp\`\",\"AutoID\":\"BETWEEN 1 AND 7\",\"IntegerShort\":\"BETWEEN 111 AND 777\",\"FloatDouble\":\"BETWEEN 111111.111111 AND 777777.777777\",\"TextShort\":\"LIKE \\\"%aa%\\\"\",\"DateTime\":\"BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07'\",\"Date\":\"BETWEEN '2021-01-01' AND '2027-07-07'\",\"Time\":\"BETWEEN '11:01:01' AND '17:07:07'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}]";
        // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1`;
        // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        // SELECT * FROM `Collection1` WHERE ((`AutoID` >= 1) AND (`IntegerShort` >= 333) AND (`FloatDouble` >= 333333.333333) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        ...
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            if (MariaDB_Client !== null) {
                ...
                // 拼接用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                SQL_String = "";  // 初始化清零;
                // SQL_String = "SELECT \`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`,\`Time_Stamp\` FROM \`Collection1\`;";
                // SQL_String = "SELECT \`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`,\`Time_Stamp\` FROM \`Collection1\` WHERE ((\`AutoID\` BETWEEN 1 AND 7) AND (\`IntegerShort\` BETWEEN 111 AND 777) AND (\`FloatDouble\` BETWEEN 111111.111111 AND 777777.777777) AND (\`TextShort\` NOT LIKE \"\%aa\%\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\')  AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\')  AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
                // SQL_String = "SELECT * FROM \`Collection1\`;";
                // SQL_String = "SELECT * FROM \`Collection1\` WHERE ((\`AutoID\` BETWEEN 1 AND 7) AND (\`IntegerShort\` BETWEEN 111 AND 777) AND (\`FloatDouble\` BETWEEN 111111.111111 AND 777777.777777) AND (\`TextShort\` NOT LIKE \"\%aa\%\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\')  AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\')  AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
                if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]' && request_POST_JSON.length > 0 && (typeof (request_POST_JSON[0]) === 'object' && Object.prototype.toString.call(request_POST_JSON[0]).toLowerCase() === '[object object]' && !(request_POST_JSON[0].length) && JSON.stringify(request_POST_JSON[0]) !== '{}')) || (typeof (request_POST_JSON) === 'object' && Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object object]' && !(request_POST_JSON.length) && JSON.stringify(request_POST_JSON) !== '{}')) {
                    ...
                    if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]') && (request_POST_JSON.length > 0)) {
                        ...
                        for (let i = 0; i < request_POST_JSON.length; i++) {
                            ...
                            // 拼接 SQL 語句字符串待檢索字段（Field）名字（Key）和取值（Value）字符串，關鍵字：WHERE 之後;
                            if (typeof (request_POST_JSON[i]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object object]' && !(request_POST_JSON[i].length)) {
                                ...
                                // // 讀取 request_POST_JSON[i] 裏關鍵字（Key）爲「"selectFieldName"」的鍵值對的值（Value）;
                                // // // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
                                // // // [*, Field-name-1, Field-name-2, Field-name-3, ... ];
                                // // let selectFieldName = [];
                                // if (request_POST_JSON[i].hasOwnProperty("selectFieldName")) {
                                //     // console.log(request_POST_JSON[i]["selectFieldName"]);
                                //     if (!((typeof (request_POST_JSON[i]["selectFieldName"]) === 'undefined') || ((typeof (request_POST_JSON[i]["selectFieldName"]) === 'object') && (request_POST_JSON[i]["selectFieldName"] === null)))) {
                                //         if (Object.prototype.toString.call(request_POST_JSON[i]["selectFieldName"]).toLowerCase() === '[object string]') {
                                //             // if ((request_POST_JSON[i]["selectFieldName"] === "") || (request_POST_JSON[i]["selectFieldName"] === "\[\]") || (request_POST_JSON[i]["selectFieldName"] === "\"\"") || (request_POST_JSON[i]["selectFieldName"] === "\'\'") || (request_POST_JSON[i]["selectFieldName"] === "\`\`")) {
                                //             //     selectFieldName = [];
                                //             // } else if ((request_POST_JSON[i]["selectFieldName"] === "\*") || (request_POST_JSON[i]["selectFieldName"] === "\[\*\]") || (request_POST_JSON[i]["selectFieldName"] === "\[\"\*\"\]") || (request_POST_JSON[i]["selectFieldName"] === "\[\'\*\'\]") || (request_POST_JSON[i]["selectFieldName"] === "\[\`\*\`\]")) {
                                //             //     selectFieldName = ["*"];
                                //             // } else {
                                //             //     selectFieldName = JSON.parse(request_POST_JSON[i]["selectFieldName"]);  // ["*", "Field-name-1", "Field-name-2", "Field-name-3", ... ];
                                //             // };
                                //             if ((String(request_POST_JSON[i]["selectFieldName"]).length > 0) && (request_POST_JSON[i]["selectFieldName"] !== "null") && (request_POST_JSON[i]["selectFieldName"] !== "NULL") && (request_POST_JSON[i]["selectFieldName"] !== "Null") && (request_POST_JSON[i]["selectFieldName"] !== "none") && (request_POST_JSON[i]["selectFieldName"] !== "NONE") && (request_POST_JSON[i]["selectFieldName"] !== "None")) {
                                //                 // 將字符串裏的反向引號（`）字符，全部刪除;
                                //                 if (request_POST_JSON[i]["selectFieldName"].indexOf("\`", 0) !== -1) {
                                //                     request_POST_JSON[i]["selectFieldName"].replace("\`", "");  // 將字符串裏的反向引號（`）字符，全部刪除;
                                //                     // request_POST_JSON[i]["selectFieldName"].replace("\`", "\"");  // 將字符串裏的反向引號（`）字符，全部替換爲雙引號（"）字符;
                                //                 };
                                //                 // 將字符串裏的雙引號（"）字符，全部刪除;
                                //                 if (request_POST_JSON[i]["selectFieldName"].indexOf("\"", 0) !== -1) {
                                //                     request_POST_JSON[i]["selectFieldName"].replace("\"", "");  // 將字符串裏的雙引號（"）字符，全部刪除;
                                //                 };
                                //                 // 將字符串裏的單引號（'）字符，全部刪除;
                                //                 if (request_POST_JSON[i]["selectFieldName"].indexOf("\'", 0) !== -1) {
                                //                     request_POST_JSON[i]["selectFieldName"].replace("\'", "");  // 將字符串裏的單引號（'）字符，全部刪除;
                                //                 };
                                //                 // 將字符串分割爲數組，使用逗號（,）字符分割;
                                //                 if ((request_POST_JSON[i]["selectFieldName"] === "\*") || (request_POST_JSON[i]["selectFieldName"] === "\"\*\"") || (request_POST_JSON[i]["selectFieldName"] === "\'\*\'") || (request_POST_JSON[i]["selectFieldName"] === "\`\*\`")) {
                                //                     selectFieldName = ["*"];
                                //                 } else if (request_POST_JSON[i]["selectFieldName"].indexOf(",", 0) !== -1) {
                                //                     selectFieldName = request_POST_JSON[i]["selectFieldName"].split(",");
                                //                 } else {
                                //                     selectFieldName = [request_POST_JSON[i]["selectFieldName"]];
                                //                 };
                                //                 // 調整數組裏指定的特殊字符串;
                                //                 if ((Object.prototype.toString.call(selectFieldName).toLowerCase() === '[object array]') && (selectFieldName.length > 0)) {
                                //                     if (selectFieldName.includes("\*") || selectFieldName.includes("\"\*\"") || selectFieldName.includes("\'\*\'") || selectFieldName.includes("\`\*\`")) {
                                //                         selectFieldName = ["*"];
                                //                     } else {
                                //                         // 將數組裏的元素的變量類型，轉換爲字符串類型;
                                //                         for (let k = 0; k < selectFieldName.length; k++) {
                                //                             // console.log(selectFieldName[k]);
                                //                             if (!((typeof (selectFieldName[k]) === 'undefined') || ((typeof (selectFieldName[k]) === 'object') && (selectFieldName[k] === null)))) {
                                //                                 selectFieldName[k] = String(selectFieldName[k]);
                                //                             };
                                //                         };
                                //                     };
                                //                 };
                                ...
                                // 拼接 SQL 語句字符串待檢索字段（Field）名字（Key）字符串，關鍵字：SELECT 之後;
                                SQL_String = "SELECT ";
                                if ((Object.prototype.toString.call(selectFieldName).toLowerCase() === '[object array]') && (selectFieldName.length > 0)) {
                                    if (selectFieldName.indexOf("*") !== -1) {
                                        ...
                                        for (let i = 0; i < selectFieldName.length; i++) {
                                            // console.log(selectFieldName[i]);
                                            if (!((typeof (selectFieldName[i]) === 'undefined') || ((typeof (selectFieldName[i]) === 'object') && (selectFieldName[i] === null)) || ((Object.prototype.toString.call(selectFieldName[i]).toLowerCase() === '[object string]') && ((selectFieldName[i] === "") || (selectFieldName[i] === "undefined") || (selectFieldName[i] === "null"))))) {
                                                // 拼接 SQL 語句字符串待檢索字段（Field）名字（Key）符串;
                                                SQL_String = SQL_String + "\`";
                                                SQL_String = SQL_String + String(selectFieldName[i]);
                                                SQL_String = SQL_String + "\`";
                                                SQL_String = SQL_String + ",";
                                            };
                                        };
                                ...
                                } else if (request_POST_JSON[i].hasOwnProperty("selectFieldName")) {
                                    if (Object.prototype.toString.call(request_POST_JSON[i]["selectFieldName"]).toLowerCase() === '[object string]') {
                                        if (String(request_POST_JSON[i]["selectFieldName"]).length > 0) {
                                            SQL_String = SQL_String + String(request_POST_JSON[i]["selectFieldName"]);
                                        } else {
                                            SQL_String = SQL_String + "*";
                                        };
                                    };
                                ...
                                // 拼接 SQL 語句字符串待新建的數據表（TABLE）名字符串，關鍵字：FROM 之後;
                                SQL_String = SQL_String + "\`";
                                SQL_String = SQL_String + String(dbTableName);
                                SQL_String = SQL_String + "\`";
                                ...
                                if (!((JSON.stringify(request_POST_JSON[i]) === '{}') || ((Object.keys(request_POST_JSON[i]).length === 1) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") || request_POST_JSON[i].hasOwnProperty("table_name") || request_POST_JSON[i].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i]).length === 2) && ((request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name")) || (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("collection_name")) || (request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i]).length === 3) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name"))))) {
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("SELECT * FROM " + "\`" + String(dbTableName) + "\`" + " WHERE (").length, 10))) {
                                            ...
                                            function(
                                                ...
                                                if (error) {
                                                    ...
                                                    for (let j = 0; j < results_Array.length; j++) {
                                                        if (results_Array[j] === 'undefined') {
                                                            // results_Array[j] = String(error.message);
                                                            results_Array[j] = [{"Error": String(error.message)}];
                                                            break;
                                                        };
                                                    };
                                                    // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String(error.message);
                                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                                    // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                                    // if (callback) { callback(response_body_String, null); };
                                                ...
                                                if (results) {
                                                    ...
                                                    for (let j = 0; j < results_Array.length; j++) {
                                                        if (results_Array[j] === 'undefined') {
                                                            results_Array[j] = results;
                                                            break;
                                                        };
                                                    };
                                                    // response_body_String = JSON.stringify(results);
                                                    // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String(JSON.stringify(results));
                                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                                    // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                                    // if (callback) { callback(null, response_body_String); };
                                        ...
                                        // results_Array.push(String("Database error."));
                                        // results_Array[i] = String("Database error.");
                                        results_Array[i] = [{"Error": String("Database error.")}];
                                        // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "Database error.";
                                        // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                        // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                        // if (callback) { callback(response_body_String, null); };
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) >= parseInt(String("SELECT * FROM " + "\`" + String(dbTableName) + "\`" + ";").length, 10))) {
                                ...
                                // results_Array.push(String("error, Request post body data unrecognizable."));
                                // results_Array[i] = String("error, Request post body data unrecognizable.");
                                results_Array[i] = [{"Error": String("error, Request post body data unrecognizable.")}];
                                // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + "error, Request post body data unrecognizable.";
                                // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                // if (callback) { callback(response_body_String, null); };
                        ...
                        // // 讀取 request_POST_JSON 裏關鍵字（Key）爲「"selectFieldName"」的鍵值對的值（Value）;
                        // // // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
                        // // // [*, Field-name-1, Field-name-2, Field-name-3, ... ];
                        // // let selectFieldName = [];
                        // if (request_POST_JSON.hasOwnProperty("selectFieldName")) {
                        //     // console.log(request_POST_JSON["selectFieldName"]);
                        //     if (!((typeof (request_POST_JSON["selectFieldName"]) === 'undefined') || ((typeof (request_POST_JSON["selectFieldName"]) === 'object') && (request_POST_JSON["selectFieldName"] === null)))) {
                        //         if (Object.prototype.toString.call(request_POST_JSON["selectFieldName"]).toLowerCase() === '[object string]') {
                        //             // if ((request_POST_JSON["selectFieldName"] === "") || (request_POST_JSON["selectFieldName"] === "\[\]") || (request_POST_JSON["selectFieldName"] === "\"\"") || (request_POST_JSON["selectFieldName"] === "\'\'") || (request_POST_JSON["selectFieldName"] === "\`\`")) {
                        //             //     selectFieldName = [];
                        //             // } else if ((request_POST_JSON["selectFieldName"] === "\*") || (request_POST_JSON["selectFieldName"] === "\[\*\]") || (request_POST_JSON["selectFieldName"] === "\[\"\*\"\]") || (request_POST_JSON["selectFieldName"] === "\[\'\*\'\]") || (request_POST_JSON["selectFieldName"] === "\[\`\*\`\]")) {
                        //             //     selectFieldName = ["*"];
                        //             // } else {
                        //             //     selectFieldName = JSON.parse(request_POST_JSON["selectFieldName"]);  // ["*", "Field-name-1", "Field-name-2", "Field-name-3", ... ];
                        //             // };
                        //             if ((String(request_POST_JSON["selectFieldName"]).length > 0) && (request_POST_JSON["selectFieldName"] !== "null") && (request_POST_JSON["selectFieldName"] !== "NULL") && (request_POST_JSON["selectFieldName"] !== "Null") && (request_POST_JSON["selectFieldName"] !== "none") && (request_POST_JSON["selectFieldName"] !== "NONE") && (request_POST_JSON["selectFieldName"] !== "None")) {
                        //                 // 將字符串裏的反向引號（`）字符，全部刪除;
                        //                 if (request_POST_JSON["selectFieldName"].indexOf("\`", 0) !== -1) {
                        //                     request_POST_JSON["selectFieldName"].replace("\`", "");  // 將字符串裏的反向引號（`）字符，全部刪除;
                        //                     // request_POST_JSON["selectFieldName"].replace("\`", "\"");  // 將字符串裏的反向引號（`）字符，全部替換爲雙引號（"）字符;
                        //                 };
                        //                 // 將字符串裏的雙引號（"）字符，全部刪除;
                        //                 if (request_POST_JSON["selectFieldName"].indexOf("\"", 0) !== -1) {
                        //                     request_POST_JSON["selectFieldName"].replace("\"", "");  // 將字符串裏的雙引號（"）字符，全部刪除;
                        //                 };
                        //                 // 將字符串裏的單引號（'）字符，全部刪除;
                        //                 if (request_POST_JSON["selectFieldName"].indexOf("\'", 0) !== -1) {
                        //                     request_POST_JSON["selectFieldName"].replace("\'", "");  // 將字符串裏的單引號（'）字符，全部刪除;
                        //                 };
                        //                 // 將字符串分割爲數組，使用逗號（,）字符分割;
                        //                 if ((request_POST_JSON["selectFieldName"] === "\*") || (request_POST_JSON["selectFieldName"] === "\"\*\"") || (request_POST_JSON["selectFieldName"] === "\'\*\'") || (request_POST_JSON["selectFieldName"] === "\`\*\`")) {
                        //                     selectFieldName = ["*"];
                        //                 } else if (request_POST_JSON["selectFieldName"].indexOf(",", 0) !== -1) {
                        //                     selectFieldName = request_POST_JSON["selectFieldName"].split(",");
                        //                 } else {
                        //                     selectFieldName = [request_POST_JSON["selectFieldName"]];
                        //                 };
                        //                 // 調整數組裏指定的特殊字符串;
                        //                 if ((Object.prototype.toString.call(selectFieldName).toLowerCase() === '[object array]') && (selectFieldName.length > 0)) {
                        //                     if (selectFieldName.includes("\*") || selectFieldName.includes("\"\*\"") || selectFieldName.includes("\'\*\'") || selectFieldName.includes("\`\*\`")) {
                        ...
                        } else if (request_POST_JSON.hasOwnProperty("selectFieldName")) {
                            if (Object.prototype.toString.call(request_POST_JSON["selectFieldName"]).toLowerCase() === '[object string]') {
                                if (String(request_POST_JSON["selectFieldName"]).length > 0) {
                                    SQL_String = SQL_String + String(request_POST_JSON["selectFieldName"]);
                                } else {
                                    SQL_String = SQL_String + "*";
                                };
                            };
                        ...
                        if (!((JSON.stringify(request_POST_JSON) === '{}') || ((Object.keys(request_POST_JSON).length === 1) && (request_POST_JSON.hasOwnProperty("selectFieldName") || request_POST_JSON.hasOwnProperty("table_name") || request_POST_JSON.hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON).length === 2) && ((request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name")) || (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("collection_name")) || (request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON).length === 3) && (request_POST_JSON.hasOwnProperty("selectFieldName") && request_POST_JSON.hasOwnProperty("table_name") && request_POST_JSON.hasOwnProperty("collection_name"))))) {
                            ...
                            if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("SELECT * FROM " + "\`" + String(dbTableName) + "\`" + " WHERE (").length, 10))) {
                                ...
                                MariaDB_Client.query(
                                    ...
                                    function(
                                        ...
                                        if (error) {
                                            ...
                                            // response_data_JSON["Database_say"] = String(error.message);
                                            response_data_JSON["Database_say"] = [{"Error": String(error.message)}];
                                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                            if (callback) { callback(response_body_String, null); };
                                        ...
                                        if (results) {
                                            ...
                                            // response_body_String = JSON.stringify(results);
                                            response_data_JSON["Database_say"] = results;
                                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                            if (callback) { callback(null, response_body_String); };
                                ...
                                console.log("Database error.");
                                // response_data_JSON["Database_say"] = "Database error.";
                                response_data_JSON["Database_say"] = [{"Error": "Database error."}];
                                response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                if (callback) { callback(response_body_String, null); };
                    ...
                    // SQL_String = "SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1`;";
                    SQL_String = "SELECT ";
                    // 拼接 SQL 語句字符串待檢索字段（Field）名字（Key）符串："SELECT * FROM" ;
                    if ((Object.prototype.toString.call(selectFieldName).toLowerCase() === '[object array]') && (selectFieldName.length > 0)) {
                        if (selectFieldName.indexOf("*") !== -1) {
                            SQL_String = SQL_String + "*";
                        } else {
        ...
        case "/countDocuments": {
        // 向 MariaDB 資料庫伺服器裏指定資料庫裏指定集合（表格）中發出檢索數據條目數量指令，並接收返回符合條件的數據的個數;
        // URL = "/countDocuments?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = { WHERE }
        // request_POST_JSON = [{ WHERE-1 }, { WHERE-1 }, ... ]
        // request_POST_String = "{\"AutoID\":\"BETWEEN 1 AND 7\",\"IntegerShort\":\"BETWEEN 111 AND 777\",\"FloatDouble\":\"BETWEEN 111111.111111 AND 777777.777777\",\"TextShort\":\"NOT LIKE \\\"%aa%\\\"\",\"DateTime\":\"BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07'\",\"Date\":\"BETWEEN '2021-01-01' AND '2027-07-07'\",\"Time\":\"BETWEEN '11:01:01' AND '17:07:07'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}";
        // request_POST_String = "[{\"AutoID\":\"BETWEEN 1 AND 7\",\"IntegerShort\":\"BETWEEN 111 AND 777\",\"FloatDouble\":\"BETWEEN 111111.111111 AND 777777.777777\",\"TextShort\":\"NOT LIKE \\\"%aa%\\\"\",\"DateTime\":\"BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07'\",\"Date\":\"BETWEEN '2021-01-01' AND '2027-07-07'\",\"Time\":\"BETWEEN '11:01:01' AND '17:07:07'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"},{\"AutoID\":\"BETWEEN 1 AND 7\",\"IntegerShort\":\"BETWEEN 111 AND 777\",\"FloatDouble\":\"BETWEEN 111111.111111 AND 777777.777777\",\"TextShort\":\"LIKE \\\"%aa%\\\"\",\"DateTime\":\"BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07'\",\"Date\":\"BETWEEN '2021-01-01' AND '2027-07-07'\",\"Time\":\"BETWEEN '11:01:01' AND '17:07:07'\",\"Currency\":\"BETWEEN 1.11 AND 7.77\"}]";
        // SELECT COUNT(*) AS RecordCount FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        // SELECT COUNT(*) AS RecordCount FROM `Collection1`;
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
                        ...
                        for (let i = 0; i < request_POST_JSON.length; i++) {
                            ...
                            if (typeof (request_POST_JSON[i]) === 'object' && Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object object]' && !(request_POST_JSON[i].length)) {
                                ...
                                if (!((JSON.stringify(request_POST_JSON[i]) === '{}') || ((Object.keys(request_POST_JSON[i]).length === 1) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") || request_POST_JSON[i].hasOwnProperty("table_name") || request_POST_JSON[i].hasOwnProperty("collection_name"))) || ((Object.keys(request_POST_JSON[i]).length === 2) && ((request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name")) || (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("collection_name")) || (request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name")))) || ((Object.keys(request_POST_JSON[i]).length === 3) && (request_POST_JSON[i].hasOwnProperty("selectFieldName") && request_POST_JSON[i].hasOwnProperty("table_name") && request_POST_JSON[i].hasOwnProperty("collection_name"))))) {
                                    ...
                                    // 拼接 SQL 語句字符串待操作數據表（TABLE）名字符串;
                                    SQL_String = "SELECT COUNT(*) AS RecordCount FROM ";
                                    SQL_String = SQL_String + "\`";
                                    SQL_String = SQL_String + String(dbTableName);
                                    SQL_String = SQL_String + "\`";
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("SELECT COUNT(*) AS RecordCount FROM " + "\`" + String(dbTableName) + "\`" + " WHERE (").length, 10))) {
                                        ...
                                        MariaDB_Client.query(
                                            ...
                                            function(
                                                ...
                                                if (results) {
                                                    ...
                                                    // results_Array.push(results[0].RecordCount);
                                                    for (let j = 0; j < results_Array.length; j++) {
                                                        if (results_Array[j] === 'undefined') {
                                                            results_Array[j] = results[0].RecordCount;
                                                            break;
                                                        };
                                                    };
                                                    // response_body_String = JSON.stringify(results);
                                                    // response_data_JSON["Database_say"] = String(response_data_JSON["Database_say"]) + "[" + String(parseInt((parseInt(i, 10) + parseInt(1, 10)), 10)) + "] - " + String(JSON.stringify(results.RecordCount));
                                                    // response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                                    // // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                                    // if (callback) { callback(null, response_body_String); };
                                    ...
                                    // SQL_String = "SELECT COUNT(*) AS RecordCount FROM `Collection1`;";
                                    // 拼接 SQL 語句字符串待操作數據表（TABLE）名字符串;
                                    SQL_String = "SELECT COUNT(*) AS RecordCount FROM ";
                                    SQL_String = SQL_String + "\`";
                                    SQL_String = SQL_String + String(dbTableName);
                                    ...
                                    if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) >= parseInt(String("SELECT COUNT(*) AS RecordCount FROM " + "\`" + String(dbTableName) + "\`" + ";").length, 10))) {
                                ...
                                MariaDB_Client.query(
                                    ...
                                    function(
                                        ...
                                        if (results) {
                                            ...
                                            // response_body_String = JSON.stringify(results);
                                            response_data_JSON["Database_say"] = String(results[0].RecordCount);
                                            response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                                            if (callback) { callback(null, response_body_String); };
        ...
        case "/sql": {
        // 向 MariaDB 數據庫發出指令執行傳入的 SQL 字符串語句，並接收返回數據;
        // URL = "/sql?Key=username:password&dbUser=user_Collection1&dbPass=user&dbName=Database1&dbTableName=Collection1";
        // request_POST_JSON = ["SQL-string-1", "SQL-string-2", ...]
        // request_POST_String = "SHOW COLUMNS FROM \`Collection1\`;";
        // request_POST_String = "[\"SHOW COLUMNS FROM \`Collection1\`;\"]";
        // request_POST_String = "SELECT \`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`,\`Time_Stamp\` FROM \`Collection1\` WHERE ((\`AutoID\` BETWEEN 1 AND 7) AND (\`IntegerShort\` BETWEEN 111 AND 777) AND (\`FloatDouble\` BETWEEN 111111.111111 AND 777777.777777) AND (\`TextShort\` NOT LIKE \"\%aa\%\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
        // request_POST_String = "[\"SELECT \`AutoID\`,\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`,\`Time_Stamp\` FROM \`Collection1\` WHERE ((\`AutoID\` BETWEEN 1 AND 7) AND (\`IntegerShort\` BETWEEN 111 AND 777) AND (\`FloatDouble\` BETWEEN 111111.111111 AND 777777.777777) AND (\`TextShort\` NOT LIKE \\\"\%aa\%\\\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));\",\"SELECT \* FROM \`Collection1\`;\"]";
        // request_POST_String = "SELECT COUNT\(\*\) AS RecordCount FROM \`Collection1\` WHERE ((\`AutoID\` BETWEEN 1 AND 7) AND (\`IntegerShort\` BETWEEN 111 AND 777) AND (\`FloatDouble\` BETWEEN 111111.111111 AND 777777.777777) AND (\`TextShort\` NOT LIKE \"\%aa\%\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
        // request_POST_String = "[\"SELECT COUNT\(\*\) AS RecordCount FROM \`Collection1\` WHERE ((\`AutoID\` BETWEEN 1 AND 7) AND (\`IntegerShort\` BETWEEN 111 AND 777) AND (\`FloatDouble\` BETWEEN 111111.111111 AND 777777.777777) AND (\`TextShort\` NOT LIKE \\\"\%aa\%\\\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));\",\"SELECT COUNT\(\*\) AS RecordCount FROM \`Collection1\`;\"]";
        // request_POST_String = "INSERT INTO \`Collection1\` \(\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`\) VALUES \(\"111\",\"111111\",\"111.111\",\"111111.111111\",\"aaa\",\"aaaaaa\",\"2021-1-1 11:01:01\",\"2021-1-1\",\"11:01:01\",\"1.11\",\"\"\),\(\"222\",\"222222\",\"222.222\",\"222222.222222\",\"bbb\",\"bbbbbb\",\"2022-2-2 12:02:02\",\"2022-2-2\",\"12:02:02\",\"2.22\",\"\"\);";
        // request_POST_String = "[\"INSERT INTO \`Collection1\` \(\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`\) VALUES \(\\\"111\\\",\\\"111111\\\",\\\"111.111\\\",\\\"111111.111111\\\",\\\"aaa\\\",\\\"aaaaaa\\\",\\\"2021-1-1 11:01:01\\\",\\\"2021-1-1\\\",\\\"11:01:01\\\",\\\"1.11\\\",\\\"\\\"\);\",\"INSERT INTO \`Collection1\` \(\`IntegerShort\`,\`IntegerLong\`,\`FloatSingle\`,\`FloatDouble\`,\`TextShort\`,\`TextLong\`,\`DateTime\`,\`Date\`,\`Time\`,\`Currency\`,\`Image\`\) VALUES \(\\\"222\\\",\\\"222222\\\",\\\"222.222\\\",\\\"222222.222222\\\",\\\"bbb\\\",\\\"bbbbbb\\\",\\\"2022-2-2 12:02:02\\\",\\\"2022-2-2\\\",\\\"12:02:02\\\",\\\"2.22\\\",\\\"\\\"\);\"]";
        // request_POST_String = "UPDATE \`Collection1\` SET \`IntegerShort\`=\"2111\",\`IntegerLong\`=\"2111111\",\`FloatSingle\`=\"2111.111\",\`FloatDouble\`=\"2111111.111111\",\`TextShort\`=\"2aaa\",\`TextLong\`=\"2aaaaaa\",\`DateTime\`=\"3021-11-11 23:23:23\",\`Date\`=\"3021-11-11\",\`Time\`=\"23:23:23\",\`Currency\`=\"9.99\" WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \"aaa\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
        // request_POST_String = "[\"UPDATE \`Collection1\` SET \`IntegerShort\`=\\\"2111\\\",\`IntegerLong\`=\\\"2111111\\\",\`FloatSingle\`=\\\"2111.111\\\",\`FloatDouble\`=\\\"2111111.111111\\\",\`TextShort\`=\\\"2aaa\\\",\`TextLong\`=\\\"2aaaaaa\\\",\`DateTime\`=\\\"3021-11-11 23:23:23\\\",\`Date\`=\\\"3021-11-11\\\",\`Time\`=\\\"23:23:23\\\",\`Currency\`=\\\"9.99\\\" WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \\\"aaa\\\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));\",\"UPDATE \`Collection1\` SET \`IntegerShort\`=\\\"2111\\\",\`IntegerLong\`=\\\"2111111\\\",\`FloatSingle\`=\\\"2111.111\\\",\`FloatDouble\`=\\\"2111111.111111\\\",\`TextShort\`=\\\"2aaa\\\",\`TextLong\`=\\\"2aaaaaa\\\",\`DateTime\`=\\\"3021-11-11 23:23:23\\\",\`Date\`=\\\"3021-11-11\\\",\`Time\`=\\\"23:23:23\\\",\`Currency\`=\\\"9.99\\\";\"]";
        // request_POST_String = "DELETE FROM \`Collection1\` WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \"aaa\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));";
        // request_POST_String = "[\"DELETE FROM \`Collection1\` WHERE ((\`AutoID\` = 1) AND (\`IntegerShort\` = 111) AND (\`FloatDouble\` = 111111.111111) AND (\`TextShort\` = \\\"aaa\\\") AND (\`DateTime\` BETWEEN \'2021-01-01 11:01:01\' AND \'2027-07-07 17:07:07\') AND (\`Date\` BETWEEN \'2021-01-01\' AND \'2027-07-07\') AND (\`Time\` BETWEEN \'11:01:01\' AND \'17:07:07\') AND (\`Currency\` BETWEEN 1.11 AND 7.77));\",\"DELETE FROM \`Collection1\`;\"]";
        // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1`;
        // SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        // SELECT COUNT(*) AS RecordCount FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
        // SELECT COUNT(*) AS RecordCount FROM `Collection1`;
        ...
        // https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
            ...
            // if (request_POST_String !== "" && isStringJSON(request_POST_String)) {
                try {
                    if ((Object.prototype.toString.call(request_POST_String).toLowerCase() === '[object string]') && (request_POST_String.length > 0)) {
                        if (((request_POST_String.slice(0, 1) === "[") && (request_POST_String.slice(-1) === "]")) || ((request_POST_String.slice(0, 1) === "{") && (request_POST_String.slice(-1) === "}"))) {
                            request_POST_JSON = JSON.parse(request_POST_String, true);
                            // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                        } else {
                            request_POST_JSON = String(request_POST_String);
                        };
                    };
                } catch (error) {
                    console.error(error);
            ...
            if (MariaDB_Client !== null) {
                ...
                // 讀取用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                if ((Object.prototype.toString.call(request_POST_JSON).toLowerCase() === '[object array]') && (request_POST_JSON.length > 0)) {
                    ...
                    for (let i = 0; i < request_POST_JSON.length; i++) {
                        ...
                        // 讀取用於發送 MariaDB 資料庫伺服器待執行的 SQL 語句字符串;
                        if ((Object.prototype.toString.call(request_POST_JSON[i]).toLowerCase() === '[object string]') && (request_POST_JSON[i] !== "")) {
                            // (request_POST_JSON[i].indexOf("=", 0) !== -1)

                            SQL_String = String(request_POST_JSON[i]);
                            console.log(SQL_String);

                            if ((MariaDB_Client !== null) && (parseInt(SQL_String.length, 10) > parseInt(String("").length, 10))) {
                    ...
                    if (request_POST_JSON !== "") {
                        // (request_POST_JSON.indexOf("=", 0) !== -1)

                        SQL_String = String(request_POST_JSON);
                        console.log(SQL_String);
                        ...
                        // console.log("error, Request post body data unrecognizable.");
                        console.log("error, Request post body data(SQL) empty.");

                        response_data_JSON["Database_say"] = "error, Request post body data(SQL) empty.";
                        response_body_String = JSON.stringify(response_data_JSON);  // 將JOSN對象轉換為JSON字符串;
                        // String = JSON.stringify(JSON); JSON = JSON.parse(String);
                        if (callback) { callback(response_body_String, null); };
...
let port = 3308;  // 1 ~ 65535 監聽端口;
let webPath = String(__dirname);  // process.cwd(), path.resolve("../"),  __dirname, __filename;  // 定義一個網站保存路徑變量;

// 連接配置 MariaDB 數據庫服務器的參數;
let MariadbHost = "[::1]";  // [::0],"0.0.0.0" or [::1],"127.0.0.1" or "localhost"; 監聽主機域名 Host domain name;
let MariadbPort = 3306;  // 1 ~ 65535 監聽端口;
let dbUser = "";  // 'administrator';  // ['root:root', 'administrator:administrator', 'admin_Database1:admin', 'user_Database1:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
let dbPass = "";  // 'administrator';  // ['root:root', 'administrator:administrator', 'admin_Database1:admin', 'user_Database1:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
// let UserPass = dbUser.concat(":", dbPass);  // 'admin_Database1:admin';  // ['root:root', 'administrator:administrator', 'admin_Database1:admin', 'user_Database1:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
let dbName = "";  // 'Database1';  // ['admin', 'Database1'];  // 定義數據庫名字變量用於儲存數據庫名，將數據庫名設為形參，這樣便於日後修改數據庫名，MariaDB 要求數據庫名稱首字母必須為大寫單數;
let MariadbUrl = "";
// MariadbUrl = `mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`;  // [`mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`];  // 定義 MariaDB 數據庫服務器入口地址，將數據庫入口地址設為形參，這樣便於日後更換數據庫服務器;
// MariadbUrl = "mariadb://admin_Database1:admin@[::1]:3306/Database1?metaAsArray=false&ssl=true&dateStrings=true&connectTimeout=1000";
if ((dbUser.length === 0) && (dbPass.length === 0)) {
    MariadbUrl = `mariadb://${ MariadbHost }:${ MariadbPort }/${ dbName }`;
} else if ((dbUser.length !== 0) || (dbPass.length !== 0)) {
    MariadbUrl = `mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`;
} else {
    MariadbUrl = `mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`;
};
// console.log(MariadbUrl);
let dbTableName = "";  // 'Collection1';  // ['Collection1'];  // MariaDB 數據庫包含的數據集合（表格）;
    ...
    for (let i = 0; i < process.argv.length; i++) {
        ...
        if (i > 1) {
            ...
            if (Object.prototype.toString.call(process.argv[i]).toLowerCase() === '[object string]' && process.argv[i] !== "" && process.argv[i].indexOf("=", 0) !== -1) {
                ...
                if (String(process.argv[i].split("=")[0]) === "MariadbHost") {
                    MariadbHost = String(process.argv[i].split("=")[1]);
                    MariadbUrl = `mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`;  // [`mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`];  // 定義 MariaDB 數據庫服務器入口地址，將數據庫入口地址設為形參，這樣便於日後更換數據庫服務器;
                };
                if (String(process.argv[i].split("=")[0]) === "MariadbPort") {
                    MariadbPort = String(process.argv[i].split("=")[1]);
                    MariadbUrl = `mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`;  // [`mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`];  // 定義 MariaDB 數據庫服務器入口地址，將數據庫入口地址設為形參，這樣便於日後更換數據庫服務器;
                };
                ...
                if (String(process.argv[i].split("=")[0]) === "dbName") {
                    if (String(process.argv[i].split("=")[1]) === "") {
                        dbName = String("");
                    } else if ((String(process.argv[i].split("=")[1]) === "none") || (String(process.argv[i].split("=")[1]) === "NONE")  || (String(process.argv[i].split("=")[1]) === "None")) {
                        dbName = String("");
                    } else if ((String(process.argv[i].split("=")[1]) === "null") || (String(process.argv[i].split("=")[1]) === "NULL")  || (String(process.argv[i].split("=")[1]) === "Null")) {
                        dbName = String("");
                    } else {
                ...
                if (String(process.argv[i].split("=")[0]) === "MariadbUrl") {
                    MariadbUrl = String(process.argv[i].split("=")[1]);
                    // MariadbUrl = "mariadb://admin_Database1:admin@[::1]:3306/Database1?metaAsArray=false&ssl=true&dateStrings=true&connectTimeout=1000";
                    dbUser = String(MariadbUrl.split("://")[1].split("@")[0].split(":")[0]);
                    dbPass = String(MariadbUrl.split("://")[1].split("@")[0].split(":")[1]);
                    MariadbHost = String(MariadbUrl.split("@")[1].split("/")[0].split(":")[0]);
                    MariadbPort = String(MariadbUrl.split("@")[1].split("/")[0].split(":")[1]);
                    // dbName = String(MariadbUrl.split("@")[1].split("/")[1].split("?")[0]);
                };
            };
        };
    };
...
// 配置文檔保存位置;
// Windows 系統：
// Global : 「System Windows Directory\my.ini」
// Global : 「Windows Directory\my.ini」
// Global : 「c:\my.ini」
// Server : 「installdir\my.ini」
// Server : 「installdir\data\my.ini」
// Server (from MariaDB 10.6) : 「%mariadb_home%\my.ini」
// Server : 「%mysql_home%\my.ini」
// Linux-Ubuntu 系統：
// Global : 「/etc/my.cnf」
// Global : 「/etc/mysql/my.cnf」
// Server (from MariaDB 10.6) : 「$MYSQL_HOME/my.cnf」
// Server (before MariaDB 10.6) : 「$MARIADB_HOME/my.cnf」
// User : 「~/.my.cnf」

// 鏈接到 MariaDB 數據庫;
// Windows 系統啓動 MariaDB 數據庫服務器示例：
// 查看幫助信息：
// C:\Database-to-Excel-VBA\MariaDB>C:\>C:\Database-to-Excel-VBA\MariaDB\MariaDB10.11\bin\mariadbd.exe --help --verbose
// 對於 Windows 系統，在 cmd 控制臺，使用如下命令：
// C:\Database-to-Excel-VBA\MariaDB>C:\Database-to-Excel-VBA\MariaDB\MariaDB10.11\bin\mariadbd.exe --defaults-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --console
...
// 即可啓動 MariaDB 數據庫的服務器端程序，並加載指定的配置文檔。鍵盤輸入：「Ctrl」+「c」，可關閉正在運行的 MariaDB 服務器端程序。
// 或者，對於 Linux-Ubuntu 系統，在 bash 控制臺命令行，使用如下命令：
// root@localhost:~# /etc/init.d/mariadb start
// 即可在後臺啓動 MariaDB 服務器端程序。
// 然後在控制臺命令行，使用如下命令：
// root@localhost:~# /etc/init.d/mariadb stop
// 在控制臺命令行，使用如下命令：
// root@localhost:~# /etc/init.d/mariadb status
// 可查看 MariaDB 服務器端程序運行狀態。
// 在控制臺命令行，使用如下命令：
// root@localhost:~# /etc/init.d/mariadb restart
// 可重新啓動正在運行的 MariaDB 服務器端程序。

// 注意，數據庫的服務器和用戶端是兩個程序，因此如果使用指令「mariadbd.exe --defaults-extra-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini"」在操作系統前臺啓動服務器，則需要另打開一個控制臺命令行窗口（例如 Windows 系統的 cmd 控制臺窗口），在新窗口中啓動數據庫用戶端程序。如果使用指令「mariadb.exe --defaults-extra-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini」在操作系統運行用戶端鏈接器，則無需另打開一個控制臺命令行窗口（例如 Windows 系統的 cmd 控制臺窗口），可以在原窗口中直接啓動數據庫用戶端程序。

// 在 Windows 系統的 cmd 控制臺命令行，使用如下命令：
// C:\Database-to-Excel-VBA\MariaDB>C:\Database-to-Excel-VBA\MariaDB\MariaDB10.11\bin\mariadb.exe --defaults-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --host=127.0.0.1 --port=3306 --user=username --password=password Database1
// C:\Database-to-Excel-VBA\MariaDB>C:\Database-to-Excel-VBA\MariaDB\MariaDB10.11\bin\mariadb.exe --defaults-extra-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --host=127.0.0.1 --port=3306 --user=username --password=password Database1
// 即可使用 MariaDB 數據庫的用戶端程序「mariadb.exe」鏈接到指定的正在運行監聽的 MariaDB 數據庫的服務器端程序。鏈接到服務器之後，指令裏手動指定了鏈接「Database1」數據庫，若指令未指定，則預設鏈接的是「none」即未鏈接任何數據庫。
// 可以在用戶端程序「mariadb.exe」的控制臺命令行，使用如下 SQL 指令字符串查看包含的所有數據庫清單：
// MariaDB [(none)]> SHOW DATABASES;
// 在用戶端程序「mariadb.exe」的控制臺命令行，使用如下 SQL 指令查看當前鏈接的數據庫：
// MariaDB [(none)]> SELECT database();
// 在用戶端程序「mariadb.exe」的控制臺命令行，使用如下 SQL 指令切換或創建當前鏈接的數據庫，例如切換到一個名爲「Database1」的數據庫，如果系統中不存在這個數據庫，則會自動創建：
// MariaDB [(none)]> USE Database1;
// 在用戶端程序「mariadb.exe」的控制臺命令行，使用如下 SQL 指令查看當前鏈接的數據庫中包含的所有集合（表格）：
// MariaDB [Database1]> SHOW TABLES;
// 在用戶端程序「mariadb.exe」的控制臺命令行，在當前鏈接的數據庫中創建指定名稱的集合（表格），例如創建一個名爲「Collection1」的集合（表格），可以使用如下 SQL 指令：
// MariaDB [Database1]> CREATE TABLE `Collection1` (`AutoID` INT AUTO_INCREMENT PRIMARY KEY,`ReplicationID` TINYINT NOT NULL,`IntegerShort` SMALLINT,`IntegerLong` INT,`FloatSingle` FLOAT,`FloatDouble` DOUBLE,`TextShort` TINYTEXT NOT NULL,`TextLong` MEDIUMTEXT,`DateTime` DATETIME,`Date` DATE,`Time` TIME,`Currency` DECIMAL(10, 2) DEFAULT 0,`Image` LONGBLOB DEFAULT NULL,`VariableBinary` VARBINARY(8) DEFAULT NULL,`Array8Byte` BINARY(8) DEFAULT NULL,`Time_Stamp` TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
// 在用戶端程序「mariadb.exe」的控制臺命令行，如果想對當前鏈接的數據庫中包含的指定集合（表格）重命名，例如對一個名爲「Collection1」的集合（表格）重命名為「new_Collection1」，可以使用如下 SQL 指令：
// MariaDB [Database1]> RENAME TABLE `Collection1` TO `new_Collection1`;
// 在用戶端程序「mariadb.exe」的控制臺命令行，如果想刪除當前鏈接的數據庫中包含的指定集合（表格），例如刪除一個名爲「test20220703」的集合（表格），可以使用如下 SQL 指令：
// MariaDB [Database1]> DROP TABLE `Collection1`;
// 在用戶端程序「mariadb.exe」的控制臺命令行，如果想刪除名爲「Database1」的數據庫，可以使用如下 SQL 指令：
// MariaDB [Database1]> DROP DATABASE `Database1`
// 在用戶端程序「mariadb.exe」的控制臺命令行，如果想創建名爲「Database1」的數據庫，可以使用如下 SQL 指令：
// MariaDB [(none)]> CREATE DATABASE `Database1`
// 在用戶端程序「mariadb.exe」的控制臺命令行，如果想從遠程主機克隆複製數據庫，可以使用如下 SQL 指令：
// MariaDB [(none)]> CREATE DATABASE `new_database_name` AS (SELECT * FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = 'source_database_name');
// 例如下實例：
// https://mariadb.com/docs/server/mariadb-quickstart-guides/mariadb-sql-cheat-cheat-guide
// SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1`;
// SELECT `AutoID`,`IntegerShort`,`IntegerLong`,`FloatSingle`,`FloatDouble`,`TextShort`,`TextLong`,`DateTime`,`Date`,`Time`,`Currency`,`Image`,`Time_Stamp` FROM `Collection1` WHERE ((`AutoID` BETWEEN 1 AND 7) AND (`IntegerShort` BETWEEN 111 AND 777) AND (`FloatDouble` BETWEEN 111111.111111 AND 777777.777777) AND (`TextShort` NOT LIKE "%aa%") AND (`DateTime` BETWEEN '2021-01-01 11:01:01' AND '2027-07-07 17:07:07')  AND (`Date` BETWEEN '2021-01-01' AND '2027-07-07')  AND (`Time` BETWEEN '11:01:01' AND '17:07:07') AND (`Currency` BETWEEN 1.11 AND 7.77));
...
// CREATE DATABASE `Database1`;
// DROP DATABASE `Database1`;
// 在用戶端程序「mariadb.exe」的控制臺命令行，使用如下命令退出用戶端程序「mariadb.exe」的控制臺命令行，返回至操作系統 Ubuntu 的控制臺命令行：
// > quit
// 或者，鍵盤輸入：「Ctrl」+「c」，也可退出用戶端程序「mongo.exe」的控制臺命令行，返回至操作系統 Ubuntu 的控制臺命令行：
// C:\Database-to-Excel-VBA\MariaDB>
...
// 參數                   描述
// ---------------------------
// host                   主機地址字符串（預設值："localhost"）
// user                   用戶名字符串
// password               密碼字符串
// port                   埠號（端口），整型（預設值：3306）
// database               資料庫名稱字符串
// charset                連接字符集，字符串類型（預設值：'UTF8_GENERAL_CI'，注意字符集的字母都要大寫）
// localAddress           資料庫伺服器 IP 地址字符串，此 IP 地址用於 TCP 鏈接時使用（可選）
// socketPath             鏈接到 unix 域路徑，當使用 host 和 port 鏈接時，會自動忽略此參數
// timezone               時區字符串（預設值：'local'）
// connectTimeout         鏈接超時（預設值：不限制，單位：毫秒）
// stringifyObjects       是否序列化物件
// typeCast               是否將列值轉化爲本地 JavaScript 類型值（預設值：true）
// queryFormat            自定義 query 語句格式化方法
// supportBigNumbers      資料庫支持 bigint 或 decimal 類型列時，需要設此 option 爲 true 值（預設值：false）
// bigNumberStrings       當 supportBigNumbers 和 bigNumberStrings 启用時，强制 bigint 或 decimal 列以 JavaScript 字符串類型返回（預設值：false）
// dateStrings            强制 timestamp, datetime, data 類型資料，以字符串類型返回，而不是 JavaScript Date 類型（預設值：false）
// debug                  開啓調試（預設值：false）
// multipleStatements     是否允許一個 query 裏包含有多個 SQL 語句（預設值：false）
// flags                  用於修改鏈接標志
// ssl                    使用 ssl 參數（與 crypto.createCredenitals 參數格式一致）或一個包含 ssl 配置文檔名稱的字符串，目前只捆綁 Amazon RDS 的配置文檔

// 創建連接器配置 MariaDB 數據庫服務器;
let MariaDB_Client = null;
// MariaDB_Client = mariadb.createConnection("mariadb://admin_Database1:admin@[::1]:3306/Database1?metaAsArray=false&ssl=true&dateStrings=true&connectTimeout=1000");
// MariaDB_Client = mariadb.createConnection(`mariadb://${ dbUser }:${ dbPass }@${ MariadbHost }:${ MariadbPort }/${ dbName }`);
// MariaDB_Client = mariadb.createConnection(MariadbUrl);
MariaDB_Client = mariadb.createConnection({
    host: MariadbHost,  // "localhost",  // process.env.DB_HOST,
    port: MariadbPort,  // 3306,
    // socketPath: "",  // Permit connecting to the database via Unix domain socket or named pipe, if the server allows it;
    ssl: false,  // The Connector can encrypt data during transfer using the Transport Layer Security (TLS) protocol. TLS/SSL allows for transfer encryption, and can optionally use identity validation for the server and client.
    database: dbName,  // "mysql",
    user: dbUser,  // "username",  // "root",  // "administrator",  // process.env.DB_USER, 
    password: dbPass,  // "password",  // "root",  // "administrator",  // process.env.DB_PWD,
    // collation: {index: 224, name: "UTF8MB4_UNICODE_CI", charset: "utf8"},
    // timezone: '+00:00',  // 'America/Los_Angeles',
    // skipSetTimezone: true,
    metaAsArray: false,
    rowsAsArray: false,
    dateStrings: true,
    connectTimeout: 1000,  // Connection timeout in milliseconds (default changed from 10000 to 1000 in 2.5.6);
    socketTimeout: 0,  // Socket timeout in milliseconds after the connection is established;
    queryTimeout: 0,  // Set maximum query time in ms (an error will be thrown if limit is reached). 0 or undefined meaning no timeout. This can be superseded for a query using timeout option;
    maxAllowedPacket: 4196304,  // permit to indicate server global variable max_allowed_packet value to ensure efficient batching. default is 4Mb.
    prepareCacheLength: 256,  // Define prepare LRU cache length. 0 means no cache;
    insertIdAsNumber: false,  // Whether the query should return last insert id from INSERT/UPDATE command as BigInt or Number. default return BigInt;
    decimalAsNumber: false,  // Whether the query should return decimal as Number. If enable, this might return approximate values.
    bigIntAsNumber: false,  // Whether the query should return BigInt data type as Number. If enable, this might return approximate values.
    supportBigNumbers: true,
    bigNumberStrings: true,
    // fullResult: false,  // indicate if user wants to retrieve individual batch results (in order to retrieve generated ids). This might change the performance of batching if set, depending on server version: if set, for server 11.5.1 and above, bulk will be use, pipelining if not;
    // logger: {
    ...
    compress: false  // Compress exchanges with database using gzip. This can give you better performance when accessing a database in a different location.
});
// 執行連接 MariaDB 數據庫服務器;
MariaDB_Client.connect(function(errConnect){

    if (errConnect) {
        console.log("MariaDB Client not connected due to error.");
        console.log(errConnect);
        console.log(`MariaDB 數據庫「${MariadbUrl}」鏈接失敗：`);
        // return 1;
    } else {
        console.log("MariaDB Client successful, connection id is", MariaDB_Client.threadId);
        console.log(`MariaDB 數據庫「${MariadbUrl}」鏈接成功.`);
        ...
        // let SQL_String = "";
        // SQL_String = "USE \`Database1\`;"  // 鏈接指定數據庫服務器中包含的指定數據庫：「Database1」的 SQL 語句字符串寫法示例;
        // // 執行指定 SQL 語句字符串;
        // MariaDB_Client.query(
        //     SQL_String,
        //     function(
        ...
        // MariaDB_Client.end(function(errClose){
        //     if (errClose) {
        //         console.log("MariaDB Client close error.");
        //         console.log(errClose.stack);
        //     } else {
        //         console.log("MariaDB Client closed.");
        //     };
        // });

        // MariaDB_Client.destroy();  // 强制關閉;
        // return 0;
    };
});
...
// // 使用示例：
// // 一、控制臺啓動 MariaDB 數據庫服務器端程序;
// // C:\Database-to-Excel-VBA\MariaDB>C:\Database-to-Excel-VBA\MariaDB\MariaDB10.11\bin\mariadbd.exe --defaults-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --console
// // C:\Database-to-Excel-VBA\MariaDB>C:\Database-to-Excel-VBA\MariaDB\MariaDB10.11\bin\mariadbd.exe --defaults-extra-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --console
// // 二、控制臺啓動自定義的鏈接 MariaDB 數據庫的 http 服務器;
// // C:\Database-to-Excel-VBA\MariaDB>C:\Database-to-Excel-VBA\Nodejs\Nodejs-22.20.0\node.exe C:/Database-to-Excel-VBA/MariaDB/NodejsToMariaDB/Nodejs2MariadbServer.js host=0.0.0.0 port=27016 number_cluster_Workers=0 Key=username:password MongodbHost=0.0.0.0 MongodbPort=27017 dbUser=administrator dbPass=administrator dbName=testWebData
// // 三、使用自定義的 http 客戶端（Client）程序，鏈接自定義的 http 服務器，進而通過 http 服務器操控 MariaDB 數據庫服務端程序;
...
// let Host = "localhost";
// let Port = "27016";
// let dbUser = 'admin_testWebData';  // ['root:root', 'administrator:administrator', 'admin_testWebData:admin', 'user_testWebData:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
// let dbPass = 'admin';  // ['root:root', 'administrator:administrator', 'admin_testWebData:admin', 'user_testWebData:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
// // let UserPass = dbUser.concat(":", dbPass);  // 'admin_test20220703:admin';  // ['root:root', 'administrator:administrator', 'admin_testWebData:admin', 'user_testWebData:user'];  // 鏈接 MariaDB 數據庫的驗證賬號密碼;
// let dbName = 'testWebData';  // ['admin', 'testWebData'];  // 定義數據庫名字變量用於儲存數據庫名，將數據庫名設為形參，這樣便於日後修改數據庫名，MariaDB 要求數據庫名稱首字母必須為大寫單數;
...
// let dbTableName = 'test20220703';  // ['test20220703'];  // MariaDB 數據庫包含的數據集合（表格）;
// let Key = "username:password";  // 自定義的訪問網站簡單驗證用戶名和密碼;
