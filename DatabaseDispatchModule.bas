...
Sub runMariaDBServerSideApplication()
    ...
    If shellPIDdict.Exists("MariaDBServerSideApplication") Then
        ...
        'codeScript = "C:/Database-to-Excel-VBA/MariaDB/MariaDB10.11/bin/mariadbd.exe --defaults-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --console"  '開啓服務器的命令行控制臺 cmd 命令全名
        codeScript = "C:/Database-to-Excel-VBA/MariaDB/MariaDB10.11/bin/mariadbd.exe --defaults-extra-file=C:/Database-to-Excel-VBA/MariaDB/configuring_mariadb_with_option_files.ini --console"  '開啓服務器的命令行控制臺 cmd 命令全名
...
Sub runMariaDBhttpServer()
    ...
    If shellPIDdict.Exists("MariaDBhttpServer") Then
        ...
        codeScript = "C:/Database-to-Excel-VBA/Nodejs/Nodejs-22.20.0/node.exe C:/Database-to-Excel-VBA/MariaDB/NodejsToMariaDB/Nodejs2MariadbServer.js host=::0 port=3308 number_cluster_Workers=0 MariadbHost=[::1] MariadbPort=3306"  '開啓服務器的命令行控制臺 cmd 命令全名
        'codeScript = "C:/Database-to-Excel-VBA/Nodejs/Nodejs-22.20.0/node.exe C:/Database-to-Excel-VBA/MariaDB/NodejsToMariaDB/Nodejs2MariadbServer.js host=::0 port=3308 number_cluster_Workers=0 MariadbHost=[::1] MariadbPort=3306 dbUser=admin_Database1 dbPass=admin dbName=none"  '開啓服務器的命令行控制臺 cmd 命令全名
        shellPIDvalue = ShellAndWait(codeScript, False)  '自定義函數，調用 Windows 的 shell 語句控制臺命令行（cmd）執行 Bash 語句或運行可執行檔（.exe）；
        '第二個參數預設值為 True，曲 True 值表示阻塞進程同步執行，同時讀取控制臺返回值，如果取 False 值，則不會阻塞進程異步執行，並且無法讀取控制臺輸出的返回值字符串，返回值只是對應的進程的 PID（ process identifier）。
...
Sub runMongoDBServerSideApplication()
    ...
    If shellPIDdict.Exists("MongoDBServerSideApplication") Then
        ...
        codeScript = "C:/Database-to-Excel-VBA/MongoDB/Server/8.2/bin/mongod.exe --config=C:/Database-to-Excel-VBA/MongoDB/NodejsToMongoDB/mongod.cfg"  '開啓服務器的命令行控制臺 cmd 命令全名
        shellPIDvalue = ShellAndWait(codeScript, False)  '自定義函數，調用 Windows 的 shell 語句控制臺命令行（cmd）執行 Bash 語句或運行可執行檔（.exe）；
        '第二個參數預設值為 True，曲 True 值表示阻塞進程同步執行，同時讀取控制臺返回值，如果取 False 值，則不會阻塞進程異步執行，並且無法讀取控制臺輸出的返回值字符串，返回值只是對應的進程的 PID（ process identifier）。
...
Sub runMongoDBhttpServer()
    ...
    If shellPIDdict.Exists("MongoDBhttpServer") Then
        ...
        codeScript = "C:/Database-to-Excel-VBA/Nodejs/Nodejs-22.20.0/node.exe C:/Database-to-Excel-VBA/MongoDB/NodejsToMongoDB/Nodejs2MongodbServer.js host=::0 port=27016 number_cluster_Workers=0 MongodbHost=[::1] MongodbPort=27017"  '開啓服務器的命令行控制臺 cmd 命令全名
        'codeScript = "C:/Database-to-Excel-VBA/Nodejs/Nodejs-22.20.0/node.exe C:/Database-to-Excel-VBA/MongoDB/NodejsToMongoDB/Nodejs2MongodbServer.js host=::0 port=27016 number_cluster_Workers=0 MongodbHost=[::1] MongodbPort=27017 dbUser=admin_Database1 dbPass=admin dbName=Database1"  '開啓服務器的命令行控制臺 cmd 命令全名
        shellPIDvalue = ShellAndWait(codeScript, False)  '自定義函數，調用 Windows 的 shell 語句控制臺命令行（cmd）執行 Bash 語句或運行可執行檔（.exe）；
        '第二個參數預設值為 True，曲 True 值表示阻塞進程同步執行，同時讀取控制臺返回值，如果取 False 值，則不會阻塞進程異步執行，並且無法讀取控制臺輸出的返回值字符串，返回值只是對應的進程的 PID（ process identifier）。
